...
;$(function () {
    $("#login_name").keyup(function(){
        $("#login_pwd").css("visibility","visible");
    })
...
//    })
    ...
    $("#user_confirm").live('click', function () {
        ...
        var str_md5 = $.md5(oldpwd);
        var flag = true;
        $.ajax({
            type: "POST",
            url: actionUrl + "fname=system&opt=login&function=set&user="+uname+"&password=" + str_md5,
            dataType: "JSON",
            async: false,
...
//    });
    ...
    //添加管理员
    $("#add_manager_btn").live("click", function () {
        //console.log($("#manager_list tr").length);
        if($("#manager_list tr").length>9){
             getMsg('最多添加9个管理员');
        }else{
            loading(1, $("#visitor_add"), 1);
            $("#visitor_add input").val('');
            $("#visitor_add label").show();
        }
    });

    //确认添加管理员
    $("#visitor_add_confirm").live('click', function () {
        var uname = trim($("#m_account").val()),
            pwd = trim($("#m_pwd").val()),
            pwd2 = trim($("#m_pwd2").val()),
            u_role=trim($("#m_type").val());
        if (uname == '') {
            getMsg('请输入帐号！', 1, '#m_account');
            return;
        }
        if (uname.length > 15) {
            getMsg('帐号长度不能超过15位！', 1, '#m_account');
            return;
        }

        if (escape(uname).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(uname)) {
            getMsg('帐号不能包含中文字符或者特殊字符！', 1, "#m_account");
            return;
        }
        if (pwd == '') {
            getMsg('请输入密码！', 1, '#m_pwd');
            return;
        }
        if (pwd.length > 15 || pwd.length < 5) {
            getMsg('密码长度不能超过15位或者小于5位！', 1, '#m_pwd');
            return;
        }
        if (pwd2 != pwd) {
            getMsg('两次密码不一致！', 1, '#m_pwd2');
            return;
        }
        if (escape(pwd).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(pwd)) {
            getMsg('密码不能包含中文字符或者特殊字符！', 1, "#m_pwd");
            return;
        }
        addUserAccount(uname,pwd,u_role);
    });
    //删除管理员
    $(".del-btn").live("click",function(){
        var index = $(this).parents("tr").attr("data-index");
        delUserAccount(index);
    })
    //修改管理员
    $(".modify-btn").live("click",function(){
        loading(1, $("#visitor_modify"), 1);
        $("#visitor_modify input").val('');
        $("#visitor_modify label").show();
        var index = $(this).parents("tr").attr("data-index"),
            username = $(this).parents("tr").attr("data-name"),
            user_role = $(this).parents("tr").attr("data-role");
        $("#m_account_modify").val(username).siblings("label").hide();
        if(user_role=="2"){
        	$("#manager_type").hide();
        	$("#user_role").val("");
        }else{
        	...
        	$("#m_type_modify").val(user_role);
        }
        $("#user_index").val(index);

    })
    /*确认修改信息*/
    $("#visitor_modify_confirm").live('click', function () {
        var index = trim($("#user_index").val()),
            uname = trim($("#m_account_modify").val()),
            pwd = trim($("#m_pwd_modify").val()),
            pwd2 = trim($("#m_pwd_modify2").val()),
            u_role;
        if($("#user_role").val()==""){
        	u_role=trim($("#user_role").val());
        }else{
        	u_role=trim($("#m_type_modify").val());
        }
        if(uname=="" && pwd=="" && pwd2=="" && u_role=="" ){
        	getMsg('请输入要修改的帐号或密码', 1, '#m_account_modify');
        	return;
        }
        if (uname == '') {
            uname='';
	        ...
	        if (uname.length > 15) {
	            getMsg('帐号长度不能超过15位！', 1, '#m_account_modify');
	            return;
	        }

	        if (escape(uname).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(uname)) {
	            getMsg('帐号不能包含中文字符或者特殊字符！', 1, "#m_account_modify");
	            return;
	        }
        }
        ...
        if (pwd == '' && pwd2=="") {
			...
			if(pwd==""){
	        	getMsg('请输入密码', 1, '#m_pwd_modify');
	        	return;
	        }
	        if (pwd.length > 15 || pwd.length < 5) {
	            getMsg('密码长度不能超过15位或者小于5位！', 1, '#m_pwd_modify');
	            return;
	        }
	        if (pwd2 != pwd) {
	            getMsg('两次密码不一致！', 1, '#m_pwd_modify2');
	            return;
	        }
	        if (escape(pwd).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(pwd)) {
	            getMsg('密码不能包含中文字符或者特殊字符！', 1, "#m_pwd_modify");
	            return;
	        }
        }
        modifyUserAccount(index,uname,pwd,u_role);
    });
    /*确认修改信息 end*/

    //确认 普通用户修改密码
    $("#manager_user_confirm").live('click', function () {
        var uname = $.cookie("user_name");
        var pwd = trim($("#m_userpwd").val());
        var oldpwd = $("#m_oldpwd").val();
        if (oldpwd == '') {
            getMsg('请输入旧密码！', 1, '#m_oldpwd');
            return;
        }
        ...
        if (flag == false) {
            getMsg('旧密码验证错误！', 1, '#m_oldpwd');
            return;
        }
        if (pwd == '') {
            getMsg('请输入密码！', 1, '#m_userpwd');
            return;
        }
        if (pwd.length > 15 || pwd.length < 5) {
            getMsg('密码长度不能超过15位或者小于5位！', 1, '#m_userpwd');
            return;
        }
        if (trim($("#m_userpwd2").val()) != pwd) {
            getMsg('两次密码不一致！', 1, '#m_userpwd2');
            return;
        }

        if (oldpwd == pwd) {
            getMsg('新密码和旧密码相同！', 1, '#m_userpwd');
            return;
        }
        if (escape(pwd).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(pwd)) {
            getMsg('密码不能包含中文字符或者特殊字符！', 1, "#m_userpwd");
            return;
        }
        setUserAccount(pwd);
    });
    //密码错误次数上限设置
    $("#pwd_count_btn").click(function(){
        var pwd_count = $("#pwd_count").val();
        if (pwd_count == '') {
            getMsg('请输入允许错误次数！', 1, '#pwd_count');
            return;
        }
        if((/^(\+|-)?\d+$/.test(pwd_count))&&pwd_count>=0){
            setPwdErrorLimit(pwd_count);
        }else{
            getMsg('输入格式错误！', 1, '#pwd_count');
            return;
        }
    })
