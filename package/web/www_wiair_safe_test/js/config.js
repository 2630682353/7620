...
function userLogin(name,str) {
    ...
    $.ajax({
        type: "POST",
        timeout: 3000,
        url: actionUrl + "fname=system&opt=login&function=set&user="+name+"&password=" + str_md5,
        dataType: "JSON",
        success: function (data) {
            // console.log(data);
            if (data.error == 0) {
                ...
                $.cookie('u_role', data.role);
                $.cookie("u_index",data.index);
                if (mob == 1) {
                    $("#account_login").hide();
                }
                if($.cookie("u_role")=="0"){
...
function showManagerList(){
    if($.cookie('u_role') == '2'){
        $(".data-manager").css("display","block");
    }else if($.cookie('u_role') == '1'){
        $(".manager").css("display","block");
    }else{
        $(".normal-manager").css("display","block");
    }
}
...
function wifiGet(type) {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=net&opt=wifi_lt" + type + "&function=get&math=" + Math.random(),
        dataType: "JSON",
        success: function (data) {
            if (data.error == 0) {
                router.getWifiAp();
...
function wifiSet(enable, time_on, week, sh, sm, eh, em, type) {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=net&opt=wifi_lt" + type + "&function=set&enable=" + enable + "&time_on=" + time_on + "&week=" + week + "&sh=" + sh + "&sm=" + sm + "&eh=" + eh + "&em=" + em + "&math=" + Math.random(),
        dataType: "JSON",
        success: function (data) {
            if (data.error == 0) {
                if (enable == 0) {
...
function toLogin() {
    ...
    $.cookie('user_name', false, {path: '/'});
    $.cookie('u_role', false, {path: '/'});
    $.cookie('u_index', false, {path: '/'});
...
function logout() {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=log_out&function=set",
        dataType: "JSON",
        success: function (data) {
            //console.log(data);
            if (data.error == 0) {
...
function setUserAccount(pwd) {
    ...
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=login_account&function=set&password=" + str_md5,
        dataType: "JSON",
        success: function (data) {
            //console.log(data);
            if (data.error == 0) {
...
function getManagerList() {
    $.post(actionUrl + 'fname=system&opt=all_account&function=get', function (data) {
        if (data.error == 0) {
            var arr=[],
                result_arr=[],
                account_info = data.account,
                index,c_name,c_index,username,user_role,user_lvl,html='';
            arr=account_info.split("},");
            for(var i =0;i<arr.length;i++){
                result_arr.push(arr[i].replace(/{/g,"").replace(/}/g,"").split(","));
            }
            // c_name = $.cookie('user_name');
            // c_index =$.cookie('u_index');
            // console.log(c_name);
            // console.log(c_index);
            // console.log("获取管理员列表返回：");
            // console.log(result_arr);
            for(var i=0;i<result_arr.length;i++){
                index = result_arr[i][0];
                username = result_arr[i][1];
                user_role = result_arr[i][2];
                if(user_role=="2"){
                    user_lvl = "超级管理员";
                }else if(user_role =="1"){
                    user_lvl = "普通管理员";
                ...
                if(username!=''){
                    if (i % 2 == 0) {
                        if(i==0){
                            html += '<tr data-index='+index+' data-role='+user_role+' data-name='+username+'><td>' + username + '</td><td>'+ user_lvl +'</td><td class="nobd"><a href="javascript:;" class="blue modify-btn">修改</a></td></tr>';
                        }
                        else{
                         html += '<tr data-index='+index+' data-role='+user_role+' data-name='+username+'><td>' + username + '</td><td>'+ user_lvl +'</td><td class="nobd"><a href="javascript:;" class="blue modify-btn">修改</a><a href="javascript:;" class="blue del-btn">删除</a></td></tr>';
                        }
                    } else {
                        html += '<tr class="even" data-index='+index+' data-role='+user_role+' data-name='+username+'><td>' + username + '</td><td>'+ user_lvl +'</td><td class="nobd"><a href="javascript:;" class="blue modify-btn">修改</a><a href="javascript:;" class="blue del-btn">删除</a></td></tr>';
                    }
                }

            }
            $("#manager_list").empty().html(html);
...
//添加管理员
function addUserAccount(uname,pwd,u_role) {
    var uname = uname,
        pwd=$.md5(pwd),
        u_role=u_role;
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=account_info&function=set&flag=add&user="+uname+"&password="+pwd+"&role="+u_role+"",
        dataType: "JSON",
        success: function (data) {
            // console.log("添加管理员返回：");
            // console.log(data);
...
//删除管理员
function delUserAccount(index) {
    var index = index;
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=account_info&function=set&index="+index+"&flag=del&user=''&password=''&role=0",
        dataType: "JSON",
        success: function (data) {
            // console.log("删除管理员返回：");
            // console.log(data);
...
//修改管理员
function modifyUserAccount(index,uname,pwd,u_role) {
    var index = index,
        uname = uname,
        pwd = pwd==""?pwd:$.md5(pwd),
        u_role=u_role;
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=account_info&function=set&index="+index+"&flag=modify&user="+uname+"&password="+pwd+"&role="+u_role+"",
        dataType: "JSON",
        success: function (data) {
            // console.log("修改管理员返回：");
             //console.log(data);
...
//首次密码提示修改
function checkFirstPwdChange(){
    var tips_layer;
    $.post(actionUrl + 'fname=system&opt=all_account&function=get', function (data) {
        if (data.error == 0) {
            // console.log(data.pwd_default);
            if(data.pwd_default=="1"){
                tips_layer = layer.tips('<a href="javascript:;" id="tips_close" style="float:right;color:#fff;margin-left:10px;">✖</a>您现在使用的是默认密码，请及时<a href="javascript:;" id="tips_change_pwd" style="color:#fff;text-decoration: underline;">修改密码</a>', '.logout', {
                  tips: [1, '#000000'],
                  time: 0
                });
            }
    ...
    $(document).on("click","#tips_change_pwd",function(){
        $(".menu a:eq(6)").click();
        layer.close(tips_layer);
    })
    $(document).on("click","#tips_close",function(){
        layer.close(tips_layer);
    })
}
//获取密码错误次数上限设置
function getPwdErrorLimit() {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=passwd_err_cnt&function=get",
        dataType: "JSON",
        success: function (data) {
            //console.log("获取密码错误次数上限设置:");
            //console.log(data.err_cnt);
            if (data.error == 0) {
                $("#pwd_count").val(data.err_cnt);
            }else{
                locationUrl(data.error);
            }
        }
...
//设置密码错误次数上限设置
function setPwdErrorLimit(count) {
    var count = count;
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=system&opt=passwd_err_cnt&function=set&err_cnt="+count+"",
        dataType: "JSON",
        success: function (data) {
            //console.log("密码错误次数上限设置:");
            //console.log(data);
            if (data.error == 0) {
                getMsg("设置成功!");
            }else{
