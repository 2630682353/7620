...
var WanDetectLink;
var dmzStatecode=false;
var dmzState=0;
var portMaclist = new Array();
/*
 * 设置PPPOE账号密码
 */
...
//家长模式切换
function ModeControl(mac,mode) {
    mac=mac.toLowerCase();
    $.ajax({
        type: "POST",
        url: actionUrl + 'fname=net&opt=host_mode&function=set&mac='+mac+'&mode='+mode,
        dataType: "JSON",
        success: function (json) {
            if (json.error == 0) {
                getMsg('操作成功！');
...
//学习模式时间
function StudyTime(mac,act,sh,sm,eh,em,week){
    $.ajax({
        type: "POST",
        url: actionUrl + 'fname=net&opt=host_study_time&function=set&mac='+mac+'&act='+act+'&sh='+sh+'&sm='+sm+'&eh='+eh+'&em='+em+'&week='+week+'&enable=1',
        dataType: "JSON",
        success: function (json) {
            if (json.error == 0) {
                getMsg('操作成功！');
...
function setdmz(mac,type,enable){
    var dat="fname=net&opt=host_nat&function=set&mac="+mac+"&act="+type+"&out_port=0&in_port=0&proto=4&enable="+enable;
    $.ajax({
        type: "POST",
        url: actionUrl + dat,
        dataType: "JSON",
...
function getdmzState(mac){
    dmzStatecode=false;
    var dat="fname=net&opt=host_nat&function=set&mac="+mac+"&act=dump"
    $.ajax({
        type: "POST",
        url: actionUrl + dat,
        async: false,
        ...
        success: function (data) {
            if (data.error == 0) {
                var natlen=data.nat.length;
                if (natlen!=0){
                    for(var i = 0; i < natlen;i++){
                        if (data.nat[i].proto==4){
                            dmzStatecode=true;
                            dmzState=data.nat[i].enable;
                        }
                    }
                }
            } else {
