...
//重新检测
    ...
    $("#ssid_confirm").live('click', function () {
        ...
        if (pwd.length > 0 && pwd.length < 6 || pwd.length > 31) {
            getMsg('无线密码不能少于6位且不大于31位！', 1, '#ssid_pwd');
            return;
        }
        layer.closeAll();
        ssid = encodeURIComponent(ssid);
    ...
    $("#wifi_zj_confirm").click(function () {
        ...
        if (!$("#have_pwd").hasClass("hide")) {
            ...
            if (wf_pwd.length > 0 && wf_pwd.length < 6 || wf_pwd.length > 31) {
                getMsg('无线密码不能少于6位且不能大于31！', 1, '#wf_pwd');
                return;
            }
        }
        wf_pwd = encodeURIComponent(wf_pwd);
    ...
    $("#lan_btn").click(function () {
        ...
        if (!$("#dhcp_onoff").hasClass('switch3 open3')&&mob==0) {
            dhcponoff = 0;
        }else if($("#dhcp_onoff").hasClass('close-btn')&&mob==1){
            dhcponoff = 0;
        }
...
//    })
    ...
    $("#wifiTimeConfirm").click(function () {
        ...
        if ((startHour*1 > endHour*1) || (startHour*1 >= endHour*1 && startMin*1 >= endMin*1)) {
...
//    });
    ...
    $("#vwifi_set_confirm").click(function () {
        var vssid = $("#wf_vnameset").val();
        var vpwd = $("#wf_vpwdset").val();
        var hidden = 0;
	...
	var terminal_mac_;  //全局保存mac
    //家长控制
    $("#devices tr").find("div.selectMode").live('click',function(){
        layer.open({
            type: 1,
            title: "家长控制设置",
            closeBtn: 0,
    	...
    	terminal_mac_ = $(this).parents("tr").attr("mac");
    });

    $("#ModeControlBtn").find("a").live('click', function(){
    	$("#ModeControlcontent").find("div.item").eq($(this).index()-1).show().siblings("div.item").hide();
    	var id_=$(this).attr("id");
    	var mode=id_=="freemode"?0:(id_=="studymode"?1:(id_=="sortmode")?2:0);
    	ModeControl(terminal_mac_,mode);
    })

    $("#SetStudyTime").click(function () {
        var startHour = $("#StudystartHour").val();
        var startMin = $("#StudystartMin").val();
        var endHour = $("#StudyendHour").val();
        var endMin = $("#StudyendMin").val();
        var week = '';
        if ($("input[name=AllstudyDay]").attr('checked')) {
            week = '1111111';
        } else {
            $("input[name=Studyday]").each(function () {
                if ($(this).attr('checked')) {
                    week += '1';
                } else {
                    week += '0';
    ...
    var chksStudyday = $("input[name='Studyday']");
    $("#AllstudyDay").click(function () {
        if ($(this).attr("checked") == "checked") {
            chksStudyday.attr("checked", "checked");
        } else {
            chksStudyday.removeAttr("checked");
        }
    });
    chksStudyday.click(function () {
        var i = 0;
        chksStudyday.each(function (index) {
            if ($(this).attr('checked')) {
                i++;
            }
        });
        if (i == 7) {
            $("#AllstudyDay").attr("checked", "checked");
        } else {
            $("#AllstudyDay").removeAttr("checked");
        }
    });

    //dmz主机设置

    $("#dmz_on").click(function(){
        if ($("#dmzselect").is(":visible")){
            return;
        }
        //alert(portMaclist[0]);
        var mac = $("#client_list_dmz").find("option:selected").attr("mac"),type="mod";
        getdmzState(mac);
        if (!(dmzStatecode)){
            type="add";
        }
    ...
    $("#dmz_off").click(function(){
        if ($("#dmzselect").is(":hidden")){
            return;
        }
        var mac = $("#client_list_dmz").find("option:selected").attr("mac"),type="mod";
        $("#dmzselect").hide();
    ...
    $("#client_list_dmz").change(function(){
        var mac=$(this).find("option:selected").attr("mac")
        setdmz(mac,"mod","1");
    })
            ...
            if (len > 0) {
                ...
                if (mac == '') {
                    ...
                    if(mob==0){
                        ...
                        $("#backList").append('<li><div>'+mac+'</div><div><a href="javascript:;" class="mpink">取消黑名单</a></div></li>');
                    }
                }
            } else {
                if(mob==0){
...
function edit_black_list(mac, index, method) {
    ...
    if(mob==0){
        ...
        var tr = $("#backList").find('li');
    }

    var len = tr.length, macList = [], list = '', macLen = 0, maxNum = 32;
    if (method == 'del') {
        maxNum = 33;
    }
    if (len > 0 && len < maxNum) {
