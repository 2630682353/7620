            ...
            if (jsonObject.error == 0) {
                ...
                if (terminals != null && terminals.length > 0) {
                    for (var i = 0; i < terminals.length; i++) {
                        portMaclist.push(terminals[i].mac);
                        var model = "";
                        if (MODEL != null) {
                ...
                $("#client_list,#client_list_dmz").html(html + html2);
                checkportMaclist();
            } else {
                locationUrl(data.error);
            }
...
function checkportMaclist(){
    var maclen=portMaclist.length;
    for(var i=0;i<maclen;i++){
        getdmzState(portMaclist[i]);
        if (dmzStatecode==true){
            $("#client_list_dmz").find("option").eq(i).attr("selected", true);
            if(dmzState==1){
                $("#dmz_on").addClass("selected");
                $("#dmz_off").removeClass("selected");
                $("#dmzselect").show();
            }
            return;
        }
...
function DynamicList(html, html2, flag, mac, name, ip, speed, up_speed, tmp_down_speed, tmp_up_speed, switch_flag, wifi_flag) {
    ...
    if (flag == 'T') {
        if (mob == 1) {
            ...
            html += "<td></td>";
            //html += "<td><div class='tbDiv selectMode'>选择模式</div></td>";
            html += '</tr>';
        }
    }
    ...
    if (flag == 'F') {
        if (mob == 1) {
            ...
            //html2 += "<td><div class='tbDiv selectMode'>选择模式</div></td>";
            html2 += '</tr>';
        }
    }
    var list = html + html2;
