    ...
    for (var i = 0; i < list.length; i++) {
        if (code == '106') {
            ...
            if (k == 1) {
                ...
                html += '<div class="tc-box"><p> 当前套餐：' + list[i].name + '</p><p>剩余时间：' + list[i].resttime + '天</p><p>开始时间：' + stime + '</p><p>结束时间：' + etime + '</p><p>套餐详情：' + list[i].content + '</p><p>连接状态：<em id="vpstatus">' + vstatus + '</em></p><div class="renew"><span class="ico"><img src="/images/coin.png"></span><a href="javascript:;" data-id=' + list[i].id + ' class="rpackageshow">续费>></a></div></div><div class="vpn-btn"><input type="button" class="btn startvpn" name="start_vpn" data-mac="' + list[i].mac + '" id=' + list[i].id + ' value=' + startButton + '></div>';
            }
        }
    }
    if (code == '106') {
                ...
                if (data.cid == '' || data.cid === undefined) {
                    if (mob == 1) {
                        loadingLay = layer.open({
                            type: 1,
                            shadeClose: false,
                            content: '<div class="loading"><p><img src="../../images/loading-2.gif"></p><p>正在连接星际互联服务器...</p></div>',
...
function system_log(type) {
    ...
    $.get(requrl, function (json) {
        if (json.error == 0) {
            if (type == 1) {
                var downlog = 'http://' + document.domain + '/logfile.gz';
                $("#exportlog").attr('href', downlog).attr('target', '_blank');
            } else {
                if (json.count > 0) {
                    var logs = json.logs;
                    var html = '';
...
//                if (set == 'get') {
//                    if (data.enable == 1) {
//                        $("#buy_on").attr('checked', true);
//                        $("#buy_off").attr('checked', false);
//                    } else {
//                        $("#buy_off").attr('checked', true);
//                        $("#buy_on").attr('checked', false);
//                    }
//                } else {
//                    if (price == 1) {
//                        getMsg('开启成功！');
