...
$(function() {
    ...
    $("#btn_1").click(function() {
        ...
        if (mtu == "") {
            ...
            if (/^[0-9]*[1-9][0-9]*$/.test(mtu) === false || mtu < 1e3 || mtu > 1492) {
    ...
    $("#btn_3").click(function() {
        ...
        if (mtu == "") {
            ...
            if (/^[0-9]*[1-9][0-9]*$/.test(mtu) === false || mtu < 1e3 || mtu > 1500) {
    ...
    $("#wifi_set_confirm").click(function() {
        ...
        if (pwd.length > 0) {
            ...
            if (escape(pwd).indexOf("%u") != -1 ||encodeURI(pwd).indexOf("%20")!="-1"|| /[\'\"{}\[\]]/.test(pwd)) {
                getMsg("密码不能包含中文字符或者特殊字符！（如：\'，\"，{，}，[，] 以及空格等）", 1, "#wf_pwdset");
                return;
            }
        }
    ...
    $("#user_confirm").live("click", function() {
        ...
        if (escape(pwd).indexOf("%u") != -1||encodeURI(pwd).indexOf("%20")!="-1" || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(pwd)) {
            getMsg("密码不能包含中文字符或者特殊字符（如：[':;*?~`!@#$%^&+={}[]<\\(),.。，]，以及空格等）！", 1, "#userpwd");
            return;
        }
        setUserAccount(pwd);
    });
