...
//        console.log(iframId);
    ...
    $('#lan_ip').bind('input propertychange', function() {
    ...
    $('#lan_ip_start').bind('input propertychange', function() {
...
//        $(this).toggleClass("close-btn");
    ...
    $("#cp-sure").click(function () {
        var url = $.cookie('url'), d = '', type = $.cookie('type'), rurl;
...
function doMethod(str, url, size, Num) {
    var cfiles, urlArray, sfile, totalMem;
    if (Num > 15) {
        getMsg('批量操作不得超过15条！');
        return;
    }
    if (str == '复制') {
        ...
        cfiles = cpmFiles(url, 1);
        urlArray = cfiles.url, totalMem = cfiles.mem;
        sfile = sameFiles(urlArray);
        if (sfile != '') {
            getMsg('存在同名文件：' + sfile);
            return;
        }
