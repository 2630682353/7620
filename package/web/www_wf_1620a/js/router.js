...
//            GwInfo('get');
    ...
    $(".layui-layer-setwin,.layui-layer-close").live("click", function () {
...
//        console.log(iframId);
    ...
    $("#lan_btn").click(function () {
        ...
        if (!$("#dhcp_onoff").hasClass('switch3 open3')&&mob==0) {
            dhcponoff = 0;
        }else if($("#swichBtn").hasClass('close-btn')&&mob==1){
            dhcponoff = 0;
        }
...
//        $(this).toggleClass("close-btn");
    ...
    $("#usb_sure").click(function () {
        ...
        if (name == '') {
            getMsg('请输入名称！', 1, '.create-inpt');
            return;
        }
        if (getStrLength(name) >= 150) {
            getMsg('名称过长！', 1, '.create-inpt');
            return;
        }
        if (/[\":*?\\/\\<\>\|\&]/.test(name)) {
            getMsg('名称包含特殊字符！', 1, '.create-inpt');
            return;
        }
        name = name.replace(/\\/g, '');
        if (type == 'mv') {
...
function doMethod(str, url, size, Num) {
    ...
    if (str == '复制') {
        ...
        layer.confirm('确定要删除么？', function (i) {
            fileRm('rm', url);
            layer.close(i);
        });
    } else if (str == '重命名') {
...
function setWifiTimeOn(type, enable) {
    ...
    if(startHour*1 > endHour*1 && time_on == 1){
    	getMsg('WIFI时间段设置错误！');
        return;
    }
    if(startHour*1 == endHour*1 && startMin*1 > endMin*1 && time_on == 1){
    	getMsg('WIFI时间段设置错误！');
       	return;
    }
...
function clearBlackList(){
        $.post(actionUrl + 'fname=net&opt=acl_black&function=set&ifindex=0&mac=empty&mac_num=0&enable=1', function (data) {
        if (data.error == 0) {
            
            if(mob==1){
                $("#backList").empty().html('<li class="liL"><div><font size="+2">暂无内容</font></div></li>');
            }else{
...
function getblack(enable){
    $.ajax({
        type: "POST",
        async: false,
        url: actionUrl + 'fname=net&opt=acl_black&function=get&ifindex=0',
        dataType: "JSON",
        success: function (json) {
            if (json.error == 0) {
                
                $.ajaxSettings.async = false;
                $.post(actionUrl + 'fname=net&opt=acl_black&function=set&ifindex=0&mac='+json.mac.join(";")+'&mac_num='+json.mac.length+'&enable='+enable, function (data) {
                    if (data.error == 0) {
                        getMsg('操作成功！');
                    }
                }, 'json');
...
$(function(){
    $("#wbswichBtn").on("click",function(){
        if($("#wbswichBtn").hasClass("close-btn")){
...
//电脑端黑名单状态
$("#blackMAC_on").live('click',function(){
    $("#wrap_clackList").show();
    getblack(1);
})
$("#blackMAC_OFF").live('click',function(){
    $("#wrap_clackList").hide();
    getblack(0);
})
