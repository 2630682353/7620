...
function setPPPOE(account, passwd, server, mac, mtu, dns, dns1, clone) {
    ...
    if (currentMode != 2 || mac != myMac || account != wanConfigJsonObject.user || passwd != wanConfigJsonObject.passwd || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || server != wanConfigJsonObject.pppoe_server || wanInfoJsonObject.connected == 0) {
...
function setStatic(ip, mask, gw, dns, dns1, mac, mtu, clone) {
    ...
    if (currentMode != 3 || mac != myMac || ip != myStaticIp || mask != myStaticMask || gw != myStaticGw || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || mtu != wanConfigJsonObject.mtu || wanInfoJsonObject.connected == 0) {
...
function intervalTime(btn, btnText, msgText, mode) {
    if (WanDetectLink == 0 && mode != 4 && mode != 5 && mode != 4 && mode != 5) {
        getMsg('离线配置成功！');
