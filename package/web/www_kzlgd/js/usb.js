...
function getTree(_this, url, width) {
    ...
    $.getJSON(actionUrl + 'fname=sys&opt=storage&function=get&type=dir&path=' + url + '&start=1&num=100', function (data) {
        if (data.error == 0) {
            ...
            if (dir_len > 0) {
                for (var i = 0; i < dir_len; i++) {
                    if (dir[i].type == 'D' && dir[i].name != '' && dir[i].name != '$RECYCLE.BIN' && dir[i].name != 'System Volume Information') {
...
function getMenuMem(url) {
    url = url.replace(/[\ /][\ /]/g, '/');
    url = encodeURI(url);
    $.ajaxSettings.async = false;
    $.getJSON(actionUrl + 'fname=sys&opt=storage&function=get&type=size&path=' + url, function (data) {
        if (data.error == 0) {
            MenuMem = data.size;
        } else {
            getMsg(getErrorCode(data.error));
