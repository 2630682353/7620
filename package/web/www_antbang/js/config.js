...
function setPPPOE(account, passwd, server, mac, mtu, dns, dns1, clone) {
    ...
    if (wanInfoJsonObject.connected == 0 || currentMode != 2 || mac != myMac || account != wanConfigJsonObject.user || passwd != wanConfigJsonObject.passwd || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || server != wanConfigJsonObject.pppoe_server || mtu != wanConfigJsonObject.mtu_pppoe) {
...
function setDHCP(mac, mtu, dns, dns1, clone) {
    ...
    if (wanInfoJsonObject.connected == 0 || currentMode != 1 || mac != myMac || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || mtu != wanConfigJsonObject.mtu) {
...
function setStatic(ip, mask, gw, dns, dns1, mac, mtu, clone) {
    ...
    if (wanInfoJsonObject.connected == 0 || currentMode != 3 || mac != myMac || ip != myStaticIp || mask != myStaticMask || gw != myStaticGw || dns != wanConfigJsonObject.dns || dns1 != wanConfigJsonObject.dns1 || mtu != wanConfigJsonObject.mtu) {
        ...
        success: function (data) {
            ...
            if (jsonObject.error == 0) {
                ...
                if (type < 1) {
                    ...
                    if (i_ == 8) {
                        clearInterval(checkWanDetectLinkInterval);
                        netTypeChoic_set(wanInfoJsonObject.mode);
                        layer.closeAll();
                    }
                } else {
                    layer.closeAll();
                    ...
                    if (jsonObject.connected == 0) {
                        if (type > 0) {
                            netTypeChoic_set(type);
                        } else {
                            netTypeChoic_set(jsonObject.mode);
                        }
                    } else {
                        toUrl();
                    }
...
function checkConfig() {
    ...
    if ($.cookie('lstatus') == 'true') {
        ...
        if (autoAccount != '' && autoAccount != null) {
            getWanInfoJsonObject();
            WanDetectLink=wanInfoJsonObject.link;
            $("#reload").show();
            $("#step_1").hide();
            $("#step_2").show();
            netTypeChoic_set(2);
            ...
            if (deviceJsonObject.config_status == 0) {	//未配置
                ...
                if (wanInfoJsonObject.link == 0) {
                    ...
                    isConnected = false;
                    layer.closeAll();
                    netTypeChoic_set(wanConfigJsonObject.mode);
                } else {
                    isConnected = true;
                    toUrl();
                }
                        ...
                        if (jsonObject.error == 0) {
                            ...
                            if (mob == 1) {
                                layer.open({
                                    ...
                                    shade: [0.7, '#000'],
                                    closeBtn: false,
                                    content: "<div class='wifiConnectLayer' style='height:auto;display:block;' id='sucLayer'><div class='bg'></div><div class='con con2'><div class='layer-suc png_ie'></div><p>获取成功</p><p>账号：<em id='account_get'></em></p><p>密码：<em id='pwd_get'></em></p><div class='box'><a id='net_work' onclick='javascript:getPPPOEAccountsuccess();' class='btn layer-btn'>去拨号</a></div></div></div>",
                                    style: 'width:90%;background-color: transparent;  box-shadow: none;'
                                });
                            } else {
                                layer.open({
...
function intervalTime(btn, btnText, msgText, mode) {
	var checkstep=$("#checkstep").val()||false;
...
function getGradeStatus() {
    ...
    $.ajax({
        ...
        success: function (doStatus) {
            if (doStatus.error == 0) {
                var content_text=$("#uploadLay");
            	if(mob==1){
            		content_text='<div class="wifiConnectLayer" id="uploadLay" style="display:block"><div class="bg"></div><div class="con"><p>固件下载</p><div id="progressBar_do"><div id="progressBar_Track"></div></div><p id="progressBarTxt"></p><p>正在下载中，请稍候</p></div></div>'
            	}
                layer.open({
                    type: 1,
                    title: false,
...
function wifiHtmlList(jsonWifiListObject) {
    ...
    if (aplist.length > 0) {
        for (var i = 0; i < aplist.length; i++) {
            ...
            var ssid=(aplist[i].ssid).toLowerCase();
            if(ssid.indexOf("script")!='-1'){
                continue;
            }
...
function black_list() {
    ...
    $.post(actionUrl + 'fname=net&opt=acl_black&function=get&ifindex=0', function (data) {
        if (data.error == 0) {
            ...
            if (len > 0) {
                for (var i = 0; i < len; i++) {
                    html += '<tr class="even"><td width="250">' + data.mac[i] + '</td><td class="nobd"><a href="javascript:;" class="pink">取消黑名单</a></td></tr>';
                }
            } else {
...
function add_black_list(list, len, mac, index) {
    ...
    $.post(actionUrl + 'fname=net&opt=acl_black&function=set&ifindex=0&mac=' + list + '&mac_num=' + len + '&enable=0', function (data) {
        if (data.error == 0) {
            getMsg('操作成功！');
            if (len > 0) {
                if (len == 1) {
                ...
                if (mac == '') {
                    ...
                    $("#backList").append('<tr class="even"><td width="250">' + mac + '</td><td class="nobd"><a href="javascript:;" class="pink">取消黑名单</a></td></tr>');
                }
            } else {
                $("#backList").empty().html('<tr class="liL"><td colspan="3"><font size="+2">暂无内容</font></td></tr>');
            }
        } else {
            locationUrl(data.error);
        }
            ...
            if (type != 1) {
                if (json.count > 0) {
                    ...
                    for (var i = 0; i < json.count; i++) {
                        ...
                        if (i % 2 == 0) {
                            if (mob == 1) {
                                html += '<li><div>' + logtime + '</div><div>' + decodeURI(logs[i].event) + '</div></li>';
                            } else {
                                html += '<tr class="even"><td width="300">' + logtime + '</td><td class="nobd">' + decodeURI(logs[i].event) + '</td></tr>';
                            }
                        } else {
                            if (mob == 1) {
                                html += '<li><div>' + logtime + '</div><div>' + decodeURI(logs[i].event) + '</div></li>';
                            } else {
                                html += '<tr><td width="300">' + logtime + '</td><td class="nobd">' + decodeURI(logs[i].event) + '</td></tr>';
                            }
                        }
                    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error == 0) {
                ...
                data.min = data.min < 10 ? '0' + data.min : data.min;
                var rtime = data.hour + ':' + data.min;
                $("#rtime").val(rtime);
                for (var i = 0; i < data.week.length; i++) {
                    $(".day-choose a").eq(data.week[i]).addClass('selected');
                }
                if (data.time_on==0){
                    $("#reload").addClass('close-btn');
                    $("#retime").hide();
                }else{
                    $("#reload").removeClass('close-btn');
                    $("#retime").show();
                }
                if(data.loop==0){
                    $("#creload").addClass('close-btn');
                }else{
                    ("#creload").removeClass('close-btn');
                }
            } else {
                locationUrl(data.error);
            }
