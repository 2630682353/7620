...
var WanDetectLink;
var Regionstate=new Array();
var TunnelsJsonObject;
var WifiinfoJsonObject;
/*
 * 设置PPPOE账号密码
 */
function setPPPOE(account, passwd, server, mac, mtu, dns, dns1, clone) {
    ...
    if (currentMode != 2 || mac != myMac || account != wanConfigJsonObject.user || passwd != wanConfigJsonObject.passwd || dns != wanConfigJsonObject.dns_pppoe || dns1 != wanConfigJsonObject.dns1_pppoe || server != wanConfigJsonObject.pppoe_server || wanInfoJsonObject.connected == 0 || mtu != wanConfigJsonObject.mtu_pppoe) {
        var data = "fname=net&opt=wan_conf&function=set&user=" + account + "&passwd=" + passwd + "&mode=2&mtu=" + mtu + "&dns=" + dns + "&dns1=" + dns1 + "&mac=" + mac + '&pppoe_server=' + server;
        $.ajax({
            type: "POST",
 ...
 * 设置PPTP/L2TP 
 */
function setPLTP(enable, type, username, password, ip, sdomain, port, mppe, act, index){
    $("#btn_5").attr("disabled", true);
    $("#btn_5").val("dial...");
    getTunnelsJsonObject();
    if (TunnelsJsonObject.tunnels.length==0&&enable==1){
        act="add";
    }
    var data="fname=net&opt=tunnel&function=set&act="+act+"&type="+type+"&enable="+enable+"&mppe="+mppe+"&serverip="+ip+"&sdomain="+sdomain+"&user="+username+"&password="+password+"&port="+port+"&index="+index;
    $.ajax({
        type: "POST",
        url: actionUrl + data + "&math=" + Math.random(),
        dataType: "json",
        success: function (data) {
            ...
            if (jsonObject.error == 0) {
                if (enable!=0){
                    getMsg('Success!');
                    setTimeout(function(){
                        window.location.href="http://" + document.domain +"/user";
                    },3000)
                }
                //$("#pltp_message").text("Enabled.")
            } else {
                locationUrl(jsonObject.error);
                return;
            }
...
//设置其他上网方式，手动关闭pptp/l2tp
function closePLTP(){
    getTunnelsJsonObject();
    if (TunnelsJsonObject.tunnels.length==0||TunnelsJsonObject.tunnels[0].enable==0) {
        return;
    }
    var tunnel = TunnelsJsonObject.tunnels[0];
    setPLTP("0", tunnel.type, tunnel.user, tunnel.password, tunnel.serverip, tunnel.sdomain, tunnel.port, tunnel.mppe, "mod", "0")
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (jsonObject.wan_link == 1) {
                ...
                checkWanDetect(jsonObject.mode);
...
function getwifiInfo(type){
    ...
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=&opt=wifi_ap"+type+"&function=get",
        async: false,
        dataType: "JSON",
        success: function (data) {
            if (data.error==0) {
                WifiinfoJsonObject=data;
            }
        }, complete: function (XHR, TS) {
            XHR = null;
        }, error: function (XHRequest, status, data) {
...
function wifiSet(type, enable, time_on, week, sh, sm, eh, em , msg) {
    $.ajax({
        type: "POST",
...
function get5GWifiAp() {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=net&opt=wifi_ap_5g&function=get&math=" + Math.random(),
        dataType: "JSON",
        success: function (data) {
            var jsonObject = data;
            if (jsonObject.error == 0) {
                ...
                var list = channel2[data.country], len = list.length, html = '';
                for (var i = 0; i < len; i++) {
                    if (jsonObject.bw==1&&list[i]==165){
                        continue;
                    }else{
                ...
                $("#wifiChannel_5g").empty().html('<option value="0">Auto</option>' + html);
        ...
        if (autoAccount != '' && autoAccount != null) {
            ...
            if (deviceJsonObject.config_status == 0) {	//未配置
                ...
                if (wanInfoJsonObject.link == 0) {
                    ...
                    layer.closeAll();
                    getTunnelsJsonObject();
                    if (TunnelsJsonObject.tunnels.length!=0&&TunnelsJsonObject.tunnels[0].enable==1) {
                        netTypeChoice(5);
                        return;
                    }
                    netTypeChoice(wanConfigJsonObject.mode);
...
function intervalTime(btn, btnText, msgText, mode) {
    closePLTP();//设置其他上网方式，手动关闭PPTP/L2TP
...
syj.ProgressBar.prototype.setPercent = function (pct) {
    ...
    if (pct < 0.99) {
        ...
        document.getElementById("_submit").value = "Confirm";
        document.getElementById("_submit").disabled = true;
        document.getElementById("msg").innerHTML = "This completes firmware update,please waiting for 3 seconds that jump to login page.";
 ...
 * 获取TunnelsJson
 */
function getTunnelsJsonObject() {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=net&opt=tunnel&function=get&math=" + Math.random(),
        async: false,
        dataType: "json",
        success: function (data) {
            var jsonObject = data;
            if (jsonObject.error == 0) {
                TunnelsJsonObject = jsonObject;
            } else {
                getMsg(getErrorCode(jsonObject.error));
                locationUrl(data.error);
                return;
...
function getTimeRe() {
    var dat = "fname=sys&opt=sys_retime&function=get";
    $.ajax({
        type: "POST",
        url: actionUrl + dat + "&math=" + Math.random(),
        dataType: "JSON",
        success: function (data) {
            if (data.error == 0) {
                $("#startHourre option[value=" + data.hour + "]").attr('selected', 'selected');
                $("#startMinre option[value=" + data.min + "]").attr('selected', 'selected');
                $("input[name=da]").each(function (index) {
                    for (var i = 0; i <= data.week.length; i++) {
                        if (index == data.week[i]) {
                            $(this).attr("checked", true);
                        }
                ...
                if (data.time_on == 1) {
                    ...
                    if (data.loop == 1) {
                        $("#wifiTimeOnree").addClass('selected');
                        $("#wifiTimeOffree").removeClass('selected');
                    } else {
                        $("#wifiTimeOffree").addClass('selected');
                        $("#wifiTimeOnree").removeClass('selected');
                    }
                } else {
                    $("#wifiTimeOffre").addClass('selected');
                    $("#wifiTimeOnre").removeClass('selected');
                    $("#wf_time_re_div").hide();
                    $("#wf_time_nav_ree").hide();
                    if (data.loop == 1) {
                        $("#wifiTimeOnree").addClass('selected');
                        $("#wifiTimeOffree").removeClass('selected');
                    } else {
...
function setTimeRe(off, loop, week, hour, min) {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=sys&opt=sys_retime&function=set&time_on=" + off + '&loop=' + loop + '&week=' + week + '&hour=' + hour + '&min=' + min + "&math=" + Math.random(),
        dataType: "JSON",
        success: function (data) {
            if (data.error == 0) {
                if (off == 0) {
    ...
    $.post(actionUrl + 'fname=net&opt=acl_black&function=get&ifindex=' + type, function (data) {
        if (data.error == 0) {
            ...
            if (len > 0) {
                if (mob==0){
                    for (var i = 0; i < len; i++) {
                        html += '<tr class="even"><td width="250">' + data.mac[i] + '</td><td class="nobd"><a href="javascript:;" class="' + cls + '">Remove</a></td></tr>';
                    }
                }else{
                    for (var i = 0; i < len; i++) {
                        html += '<li ><div>' + data.mac[i] + '</div><div><a href="javascript:;" class="mbpink">Remove</a></div></li>';
                    }           
                }
            }
            ...
            if (html == '') {
                if(mob==0){
                    html = '<tr class="liL"><td colspan="3"><font size="+2">Empty list</font></td></tr>';
                }else{
                    html = '<li class="liL"><h3>Empty list</h3></li>';
                }
            }
...
function add_black_list(list, len, mac, index, type) {
    ...
    var cls = 'pink',
        liHtml = '<tr class="liL"><td colspan="3"><font size="+2">Empty list</font></td></tr>',
        mobliHtml = '<li class="liL"><h3>Empty list</h3></li>';
        ...
        if (data.error == 0) {
            ...
            if (len > 0) {
                ...
                if (mac == '') {
                    ...
                    var appendhtml = '<tr class="even"><td width="250">' + mac + '</td><td class="nobd"><a href="javascript:;" class="' + cls + '">Remove</a></td></tr>';
                    var mobappendhtml='<li><div>'+mac+'</div><div><a href="javascript:;" class="mbpink">Remove</a></div></li>';
                    if(mob==0){
                        if (type == 1) {
        ...
        if (data.error == 0) {
            ...
            if (len > 0) {
                if (mob==0){
                    ...
                    for (var i = 0; i < len; i++) {
                        html += '<li ><div>' + data.mac[i] + '</div><div><a href="javascript:;" class="mwpink">Remove</a></div></li>';
                    }           
                }
            }
            if (html == '') {
...
function add_white_list(list, len, mac, index, type) {
    var cls = 'wpink', liHtml = '<tr class="liL"><td colspan="3"><font size="+2">Empty list</font></td></tr>',mobliHtml = '<li class="liL"><h3>Empty list</h3></li>';
    if (type == 1) {
        cls = 'pink2';
    }
    $.ajaxSettings.async = false;
        ...
        if (data.error == 0) {
            ...
            if (len > 0) {
                ...
                if (mac == '') {
                    ...
                    var appendhtml = '<tr class="even"><td width="250">' + mac + '</td><td class="nobd"><a href="javascript:;" class="' + cls + '">Remove</a></td></tr>';
                    var mobappendhtml='<li><div>'+mac+'</div><div><a href="javascript:;" class="mwpink">Remove</a></div></li>';                    
...
var _switchMAC = {
    ...
    getblackSwitch:function(){ //Remove状态
