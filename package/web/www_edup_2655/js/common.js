...
/*
 * 检查LANIP是否合法(必须先验证ip是否合法)
 */
function checkLANIP(ip) {
    var ipArr = ip.split(/\./,4);
    if(ip=='0.0.0.1' || ip=='224.1.1.1' || ip=='240.1.1.1' || ip=='255.1.1.1' || ip=='127.0.0.1' || (ipArr[0] == '0' && ipArr[3] == '0') || ipArr[3]=='255'){
        return false;
    }else{
        return true;
    }
...
function isEqualIPAddress(addr1, addr2, mask) {
    if (!addr1 || !addr2 || !mask) {
//        getMsg("Parameters cannot be empty");
        return false;
    }
    var res1 = [], res2 = [];
    addr1 = addr1.split(".");
...
function showPLTP(){
    if (typeof (TunnelsJsonObject)=='undefined') {
        getTunnelsJsonObject();
    }
    if (TunnelsJsonObject.tunnels.length==0) {
        return;
    }
    var tunnel = TunnelsJsonObject.tunnels[0];
    $("#pl_type").val(tunnel.type);
    $("#pl_username").val(tunnel.user).siblings("label").hide();
    $("#pl_password").val(tunnel.password).siblings("label").hide();
    if (tunnel.serverip!=""){
        $("#pl_sstype").val("0");
        $("#pl_ssvalue").val(tunnel.serverip).siblings("label").hide();
    }else{
        $("#pl_sstype").val("1");
        $("#pl_ssvalue").val(tunnel.sdomain).siblings("label").hide();
    }
    $("#pl_port").val(tunnel.port).siblings("label").hide();
    if (tunnel.mppe==0) {
        $("#pl_mppe").removeClass("open3");
    }
}
 ...
 * 设置wifi账户密码
 */
function setWifiAp(type, ssid, pwd, country, channel, hidden_ssid, wifiBandwidth,msg) {
    var dat = "fname=net&opt=wifi_ap" + type + "&function=set&ssid=" + ssid + "&passwd=" + pwd + "&hidden=" + hidden_ssid + "&channel=" + channel + "&bw=" + wifiBandwidth + "&country="+ country;
    $.ajax({
        type: "POST",
        url: actionUrl + dat,
        dataType: "json",
            ...
            success: function (data) {
                ...
                if (jsonObject.error == 0) {
                    ...
                    if (TunnelsJsonObject.tunnels.length!=0&&TunnelsJsonObject.tunnels[0].enable==1) {
                        $("#netType").text(TunnelsJsonObject.tunnels[0].type.toUpperCase());
                        jsonObject.ip=TunnelsJsonObject.tunnels[0].ip;
                    }else{
                    ...
                    if (terminals != null && terminals.length > 0) {
                        for (var i = 0; i < terminals.length; i++) {
                            ...
                            if (terminals[i].flag.substr(1, 1) == 'T') {
                                ...
                                html += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + terminals[i].mac.replace(/:/g, "") + "' value='" + device_name + "' title='" + device_name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + device_name + "' title='" + device_name + "'></span><span class='small'>" + terminals[i].mac + "</span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + terminals[i].ip + "</span></div></td>";
                            ...
                            if (terminals[i].flag.substr(1, 1) == 'F') {
                                ...
                                html2 += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + terminals[i].mac.replace(/:/g, "") + "' value='" + device_name + "' title='" + device_name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + device_name + "' title='" + device_name + "'></span><span class='small'>" + terminals[i].mac + "</span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + terminals[i].ip + "</span></div></td>";
    ...
    getWifiAp: function () {
        $.ajax({
            ...
            success: function (data) {
                var jsonObject = data;
                if (jsonObject.error == 0) {
                    $("#country option[value="+jsonObject.country+"]").attr("selected", "selected");
                    ...
                    var list = channel[data.country], len = list.length, html = '';
                    for (var i = 0; i < len; i++) {
                        html += '<option value="' + list[i] + '">' + list[i] + '</option>';
                    }

                    $("#wifiChannel").empty().html('<option value="0">Auto</option>' + html);
                    $("#check_channel").empty().html(html);
...
function setdefaultWifiAp(country) {
    $.ajax({
        type:"POST",
        async: false,
        ...
        success:function(data){
            $.ajax({
                type:"POST",
                async: false,
                url:actionUrl + "fname=net&opt=wifi_ap&function=set&ssid="+data.ssid+"&passwd="+data.passwd+"&hidden="+data.hidden+"&channel=0&bw="+data.bw+"&country="+country,
                dataType: "json",
