...
//        if ($("#u_pwd").val() == '') {
//            getMsg('请输入密码', 1, '#u_pwd');
//            return;
//        }
        var str = $("#u_pwd").val();
        $("#u_login").text('正在登录');
        $("#u_login").attr("disabled", true);
...
//    })
    ...
    $("#user_confirm").click(function () {
        ...
        setUserAccount(pwd, 1);
    });

    //修改U口密码
    $("#u_confirm").click(function () {
        var oldpwd = $("#uoldpwd").val();
//        if (oldpwd == '') {
//            getMsg('请输入旧密码！', 1, '#uoldpwd');
//            return;
//        }
        ...
        $.ajax({
            type: "POST",
            url: actionUrl + "fname=sys&opt=usb_login&function=get&user=admin&password=" + oldpwd,
            dataType: "JSON",
            async: false,
            success: function (data) {
                if (data.error != 0) {
        ...
        if (flag == false) {
            getMsg('旧密码验证错误！', 1, '#uoldpwd');
            return;
        }
        var pwd = $("#uuserpwd").val();
//        if (pwd == '') {
//            getMsg('请输入密码！', 1, '#uuserpwd');
//            return;
//        }
        if ($("#uuserpwd2").val() != pwd) {
            getMsg('两次密码不一致！', 1, '#uuserpwd2');
            return;
        }

        if (oldpwd == pwd) {
            getMsg('新密码和旧密码相同！', 1, '#uuserpwd');
            return;
        }

        if (pwd.length > 15 || pwd.length < 5) {
            getMsg('密码长度不能超过15位或者少于5位！');
            return;
        }

        if (escape(pwd).indexOf("%u") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(pwd)) {
            getMsg('密码不能包含中文字符或者特殊字符！', 1, "#uuserpwd");
            return;
        }
        setUserAccount(pwd, 2);
    });
...
function loading(num, content_text, type) {
    ...
    if (type == 1) {
        ...
        content = '<div class="loading"><p><img src="../images/loading-' + num + '.gif" width="' + width + '"></p><p>' + content_text + '</p></div>';
    }
    layer.open({
        type: 1,
        title: false,
