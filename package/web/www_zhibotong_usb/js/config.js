...
function userLogin(str, type) {
    var url;
    if (type == 1) {
        str = $.md5('admin' + str);
        url = "fname=system&opt=login&function=set&usrid=" + str + "&math=" + Math.random();
    } else {
        url = "fname=sys&opt=usb_login&function=get&user=admin&password=" + str + "&math=" + Math.random();
    }
    $.ajax({
        type: "POST",
        url: actionUrl + url,
        ...
        success: function (data) {
            if (data.error == 0) {
                if (type == 1) {
                    ...
                    $.cookie('ustatus', true);
                    document.location = 'http://' + document.domain + '/app/explorer/explorer.html';
                }
...
//设置路由登录账户
function setUserAccount(pwd, type) {
    var url;
    if (type == 1) {
        url = "fname=system&opt=login_account&function=set&user=admin" + "&password=" + pwd;
    } else {
        url = "fname=sys&opt=usb_login&function=set&user=admin" + "&password=" + pwd;
    }
    $.ajax({
        type: "POST",
        url: actionUrl + url,
        ...
        success: function (data) {
            if (data.error == 0) {
                if (type == 1) {
                    ...
                    $("#uoldpwd").val('').siblings("label").show();
                    $("#uuserpwd").val('').siblings("label").show();
                    $("#uuserpwd2").val('').siblings("label").show();
                    getMsg("修改成功！");
                }
            } else {
                locationUrl(data.error);
