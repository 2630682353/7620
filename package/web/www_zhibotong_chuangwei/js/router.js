...
//重新检测
    ...
    $("#btn_1").click(function () {
        ...
        /*拨号设置WiFi*/
        var wifiAccount=$("#wf_nameset_2").val()||"";
        var wifiPasswd=$("#wf_pwdset_2").val()||"";
        if(wifiAccount.length>0){

            if (getStrLength(wifiAccount) > 31 ||escape(wifiPasswd).indexOf("%wifiAccount") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(wifiAccount)){
                getMsg("无线名称长度不得超过31位字符或者包含特殊字符！", 1, '#wf_nameset_2');
                return;
            }

            if (wifiPasswd.length > 0) {
                if (wifiPasswd.length > 31 || wifiPasswd.length < 8) {
                    getMsg("密码长度不能超过31位或不能少于8位！",1,"#wf_pwdset_2");
                    return;
                }
                if (escape(wifiPasswd).indexOf("%u") != -1 ||escape(wifiPasswd).indexOf("%20") != -1 || /[\'\"{}\[\]]/.test(wifiPasswd)) {
                   getMsg("密码不能包含中文字符或者特殊字符！",1,"#wf_pwdset_2");
                   return;
                }
            }
            firstSetWifi(wifiAccount,wifiPasswd,"#wf_nameset_2","#wf_pwdset_2");
        }
        /*拨号设置WiFi*/

        if(/.*[\u4e00-\u9fa5]+.*$/.test(account)){
            getMsg("账号不能包含中文！", 1, '#acc');
            return;
        }

        if(/.*[\u4e00-\u9fa5]+.*$/.test(account)){
            getMsg("密码不能包含中文！", 1, '#pwd');
            return;
        }
    ...
    $("#btn_3").click(function () {
        ...
        /*DHCP设置WiFi*/
        var wifiAccount=$("#wf_nameset_1").val()||"";
        var wifiPasswd=$("#wf_pwdset_1").val()||"";
        if(wifiAccount.length>0){

            var wifiAccount=$("#wf_nameset_1").val();
            var wifiPasswd=$("#wf_pwdset_1").val();
            if (getStrLength(wifiAccount) > 31 ||escape(wifiAccount).indexOf("%20") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(wifiAccount)){
                getMsg("无线名称长度不得超过31位字符或者包含特殊字符！", 1, '#wf_nameset_1');
                return;
            }
            if (wifiPasswd.length > 0) {
                if (wifiPasswd.length > 31 || wifiPasswd.length < 8) {
                    getMsg("密码长度不能超过31位或不能少于8位！",1,"#wf_pwdset_1");
                    return;
                }
                if (escape(wifiPasswd).indexOf("%u") != -1 ||escape(wifiPasswd).indexOf("%20") != -1 || /[\'\"{}\[\]]/.test(wifiPasswd)) {
                   getMsg("密码不能包含中文字符或者特殊字符！",1,"#wf_pwdset_1");
                   return;
                }
            }
            firstSetWifi(wifiAccount,wifiPasswd,"#wf_nameset_1","#wf_pwdset_1")
        }
        /*DHCP设置WiFi*/
    ...
    $("#btn_2").click(function () {
        ...
        /*DHCP设置WiFi*/
        var wifiAccount=$("#wf_nameset_3").val()||"";
        var wifiPasswd=$("#wf_pwdset_3").val()||"";
        if(wifiAccount.length>0){
            if (getStrLength(wifiAccount) > 31 ||escape(wifiAccount).indexOf("%20") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(wifiAccount)){
                getMsg("无线名称长度不得超过31位字符或者包含特殊字符！", 1, '#wf_nameset_3');
                return;
            }
            if (wifiPasswd.length > 0) {
                if (wifiPasswd.length > 31 || wifiPasswd.length < 8) {
                    getMsg("密码长度不能超过31位或不能少于8位！",1,"#wf_pwdset_3");
                    return;
                }
                if (escape(wifiPasswd).indexOf("%u") != -1 ||escape(wifiPasswd).indexOf("%20") != -1 || /[\'\"{}\[\]]/.test(wifiPasswd)) {
                   getMsg("密码不能包含中文字符或者特殊字符！",1,"#wf_pwdset_3");
                   return;
                }
            }
            firstSetWifi(wifiAccount,wifiPasswd,"#wf_nameset_3","#wf_pwdset_3")
        }
        /*DHCP设置WiFi*/
        ...
        if (!checkIP(static_ip)||getIPArray(static_ip)[3]>249) {
    ...
    /*开机配置wifi*/
    function firstSetWifi(ssid, pwd,ssidBox,pwdBox) {
        var dat = "fname=net&opt=wifi_ap&function=set&ssid=" + encodeURIComponent(ssid) + "&passwd=" + encodeURIComponent(pwd) + "&hidden=0&channel=0&bw=1";
        $.ajax({
            type: "POST",
            url: actionUrl + dat,
            dataType: "json",
            success: function (data) {
                if (data.error == 0) {
                    $(ssidBox).text(data.ssid).siblings("label").hide();
                    $(pwdBox).text(data.pwd).siblings("label").hide();
                } else {
    ...
    $("#wifi_zj_confirm").click(function () {
        ...
        /*无线中继设置WiFi*/
        var wifiAccount=$("#wf_nameset_4").val()||"";
        var wifiPasswd=$("#wf_pwdset_4").val()||"";
        
        if(wifiAccount.length>0){
            if (getStrLength(wifiAccount) > 31 ||escape(wifiAccount).indexOf("%20") != -1 || /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(wifiAccount)){
                getMsg("无线名称长度不得超过31位字符或者包含特殊字符！", 1, '#wf_nameset_4');
                return;
            }
            if (wifiPasswd.length > 0) {
                if (wifiPasswd.length > 31 || wifiPasswd.length < 8) {
                    getMsg("密码长度不能超过31位或不能少于8位！",1,"#wf_pwdset_4");
                    return;
                }
                if (escape(wifiPasswd).indexOf("%u") != -1 ||escape(wifiPasswd).indexOf("%20") != -1 || /[\'\"{}\[\]]/.test(wifiPasswd)) {
                   getMsg("密码不能包含中文字符或者特殊字符！",1,"#wf_pwdset_4");
                   return;
                }
            }

            firstSetWifi(wifiAccount,wifiPasswd,"#wf_nameset_4","#wf_pwdset_4")
        }
        /*无线中继设置WiFi*/
...
/**暂时处理**/
    ...
    $("#lan_btn").click(function () {
        ...
        var lan_time = $("#lan_time").val();
        var wan_ip=$("#wanIP").text();
        var IPArray = getIPArray(ip);
        ...
        if(wan_ip){
        	...
        	var mob_Wanip=getIPArray(mobWanip);
        	var _mobWanip=mob_Wanip[0]+"."+mob_Wanip[1]+"."+mob_Wanip[2];
        	if(_iparray==_mobWanip){
        		//alert("IP地址与上层路由IP冲突！");
        ...
        if (!isEqualIPAddress(ip_start, ip_end, mask)) {
            getMsg("终止IP地址与起始IP地址不在同一网段！", 1, '#lan_ip_end');
            return;
        }
        ...
        if (lan_time>600){
            getMsg("租约时间超出限制！", 1, '#lan_time');
            return;
        }

        if (!(/^[0-9]*[1-9][0-9]*$/.test(lan_time))) {
            getMsg("租约时间请设置整小时！", 1, '#lan_time');
            return;
        }
        ...
        if(isNaN(lan_time)){
        	...
        	var mob = getMobile();
        	if(mob==1){
        		getMsg("租约请填写数字，不填默认为1");
        	}else{
        		getMsg("租约请填写数字，不填默认为1", 1, '#lan_time');
        ...
        loading(3, '正在配置...');
        lan_time=lan_time||1;
        setLanDHCP(ip, mask, ip_start, ip_end, dhcponoff, ipMark, lan_dns1, lan_dns2,lan_time);
    });
...
//    $("#wf_time_nav_re a").click(function () {
//        $("#wf_time_re_div").toggle();
//    });
...
//        }
    ...
    $('#PpoeHightSet').click(function () {
        $("#t_1_box ul.pp").find('li').each(function (index) {
            if (index > 1) {
    ...
    $('#StaticHightSet').click(function () {
        $("#t_2_box ul.stat").find('li').each(function (index) {
            if (index > 3) {
                if ($(this).is(':visible') == false) {
                    $(this).show();
    ...
    $('#DhcpHightSet').click(function () {
        $("#t_3_box ul.dh").find('li').each(function (index) {
            if ($(this).is(':visible') == false) {
                $(this).show();
...
//    });
    ...
    $("#vwifi_set_confirm").click(function () {
        ...
        if (getStrLength(vssid) > 31 ||encodeURI(vssid).indexOf("%20")!="-1"|| /[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(vssid)) {
            getMsg('无线名称长度不得超过31位字符或者包含特殊字符！', 1, '#wf_nameset');
            return;
        }
        ...
        if (vpwd.length > 0) {
            ...
            if (escape(vpwd).indexOf("%u") != -1 ||encodeURI(vpwd).indexOf("%20")!="-1"|| /[\'\"{}\[\]]/.test(vpwd)) {
...
$(function(){
    ...
    $("#clearblack").click(function () {
        if (mob == 1) {
            layer.open({
                content: '清除所有黑名单并关闭黑名单功能？',
                btn: ['确认', '取消'],
                shadeClose: false,
                yes: function () {
                    _setSwitch("black");
                    $("#backList").find("li").remove();
                    $('<li class="liL"><h3>暂无内容</h3></li>').appendTo("#backList");
                    layer.closeAll();
                    getMsg("操作成功！");
                }
            });
        } else {
            layer.confirm('清除所有黑名单并关闭黑名单功能？', function (i) {
                _setSwitch("black");
                layer.close(i);
                $("#backList").find("tr").remove();
                $('<tr class="liL"><td colspan="3"><font size="+2">暂无内容</font></td></tr>').appendTo("#backList");
    ...
    $("#clearwhite").click(function () {
        if (mob == 1) {
            layer.open({
                content: '清除所有白名单并关闭白名单功能？',
                btn: ['确认', '取消'],
                shadeClose: false,
                yes: function () {
                    _setSwitch("white");
                    $("#whiteList").find("li").remove();
                    $('<li class="liL"><h3>暂无内容</h3></li>').appendTo("#whiteList");
                    layer.closeAll();
                    getMsg("操作成功！");
                }
            });
        } else {
            layer.confirm('清除所有白名单并关闭白名单功能？', function (i) {
                _setSwitch("white");
                layer.close(i);
                $("#whiteList").find("tr").remove();
                $('<tr class="liL"><td colspan="3"><font size="+2">暂无内容</font></td></tr>').appendTo("#whiteList");
                $("#open_black_white").removeClass("selected");
                $("#close_black_white").addClass("selected");
                $("#wrap_whiteMAC_list").hide();
            });
    ...
    $("#wbswichBtn").click(function(){
        ...
        layer.open({
            ...
            content: '<div class="wifiConnectLayer" id="black_white_layer" style="display:block;"><div class="bg"></div><div class="con"><p>请选择添加黑名单/白名单？</p><div class="box"><a href="javascript:;" class="btn layer-btn openblack">黑名单</a>&nbsp;<a href="javascript:;" class="btn layer-btn openwhite">白名单</a></div></div></div>',
