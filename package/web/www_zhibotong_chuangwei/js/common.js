...
function getMsg(msg, type, sid) {
    var mob = getMobile();
    if (mob == 1) {
        return tips(msg,{time:4000});
    } else if (type == 1 && mob != 1) {
        layer.tips(msg, sid, {
            tips: [1, '#000000']
        });
    } else {
        return layer.msg(msg,{time:4000});
    }
}
 ...
 * 设置wifi账户密码
 */
function setWifiAp(ssid, pwd, channel, hidden_ssid, wifiBandwidth, type) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error == 0) {
                ...
                /*20160831 显示wifi全称*/
                //if (getStrLength(ssid) > 23) {
                //    ssid = decodeURIComponent(ssid);
                //    $('#wifi_name').attr('title', ssid).text(ssid.substr(0, 20) + '...');
                //} else {
                    $("#wifi_name").text(decodeURIComponent(ssid));
                //}
                $("#wifi_password").text(decodeURIComponent(pwd));
            } else {
                ...
                if (terminals != null && terminals.length > 0) {
                    for (var i = 0; i < terminals.length; i++) {
                        ...
                        if (terminals[i].flag.substr(6, 1) == 'T') {
                            ...
                            if (/[\':;*?~`!@#$%^&+={}\[\]\<\\(\),\.\。\，]/.test(terminals[i].name)) {
                                device_name = model + "未识别";
                            }else{
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (jsonObject.error == 0) {
                    ...
                    //if (getStrLength(jsonObject.ssid) > 23) {
                    //    $('#wifi_name').attr('title', jsonObject.ssid).text(jsonObject.ssid.substr(0, 20) + '...');
                    //} else {
                        $('#wifi_name').text(decodeURI(jsonObject.ssid));
                    //}
                    $('#wifi_name2').text(decodeURI(jsonObject.ssid));
                    $('.wf_nameset').val(decodeURI(jsonObject.ssid)).siblings("label").hide();
                    $('#wf_nameset').val(decodeURI(jsonObject.ssid)).siblings("label").hide();
    ...
    getBusinessPic: function () {
        $.ajax({
            ...
            success: function (data) {
                var jsonObject = data;
                if (jsonObject.error == 0) {
                    mtd_param = jsonObject.value;
                    if (jsonObject.value == 1) {
                        $("#showBusinessPic").show();
                        $("#wavapp").show();
                        $("#wavshow").attr('stype', '');
                        $("#businessPic").attr("src", "../images/erweima.png").parent("a").attr("href", "http://apps.wavlink.com/ry/down.html").attr("target", "_blank");
                    } else if (jsonObject.value == 2) {
                        $("#showBusinessPic").show();
                        $("#businessPic").attr("src", "../images/erweima2.png").parent("a").attr("href", "#").attr("target", "");
                    }
                } else if (jsonObject.error == '10001') {
                    mtd_param = 1;
                    $("#showBusinessPic").show();
                    $("#wavapp").show();
                    $("#wavshow").attr('stype', '');
...
function DynamicList(html, html2, flag, mac, name, ip, speed, up_speed, tmp_down_speed, tmp_up_speed, switch_flag, wifi_flag) {
    ...
    if (flag == 'T') {
        if (mob == 1) {
            ...
            html += '<tr mac="' + mac + '">';
            html += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + mac.replace(/:/g, "") + "' value='" + name + "' title='" + name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + name + "' title='" + name + "'></span><span class='small'>" + mac + "</span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + ip + "</span></div></td>";
    ...
    if (flag == 'F') {
        if (mob == 1) {
            ...
            html2 += '<tr mac="' + mac + '">';
            html2 += "<td><div class='tbDiv'><span class='name'><input type='hidden' id='h_nick_" + mac.replace(/:/g, "") + "' value='" + name + "' title='" + name + "'><input type='text' readonly='readonly' class='tbInpt nameInpt' value='" + name + "' title='" + name + "'></span><span class='small'>" + mac + "</span><span class='small'><i class='tbicon-1 " + wifi_flag + "'></i>" + ip + "</span></div></td>";
...
function checkeLink(){
    intervalcheckeLink=setInterval(function(){
        if($("#lineSet").hasClass("selected")){
            if($('#btn_1').attr("disabled") == "disabled"){
                return;
            }
            ...
            clearInterval(intervalcheckeLink);
        }
    },10000);
}
