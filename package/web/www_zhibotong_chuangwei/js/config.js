...
function checkConfig() {
    if ($.cookie('lstatus') == 'true') {
        ...
        if (autoAccount != '' && autoAccount != null) {
            ...
            var li = $("#t_1_box ul.pp").find('li');

            if (wanConfigJsonObject.dns_pppoe == '' && wanConfigJsonObject.dns1_pppoe == '') {
            ...
            if (deviceJsonObject.config_status == 0) {	//未配置
                ...
                } else if (wanInfoJsonObject.connected == 0) { //不能上网==>拨号
                    //isConnected = false;
                    //layer.closeAll();
                    //alert(wanInfoJsonObject.ip);
                    //if(wanInfoJsonObject.ip=="0.0.0.0"||wanInfoJsonObject.ip==""||wanInfoJsonObject.ip=="undefined"||wanInfoJsonObject.ip===null){
                    //    netTypeChoice(2);
                    //}else{
                    //    netTypeChoice(wanConfigJsonObject.mode);
                    //}
                    
                //} else if(wanInfoJsonObject.connected == 1){//能上网==>动态
                    isConnected = false;
                    layer.closeAll();
                    netTypeChoice(wanConfigJsonObject.mode);
                } else {
...
function checkSetting() {
    ...
    if (userLoginStatus == "true") {
        ...
        if (wanInfoJsonObject.link==0){
            $("#pppoe_message,#static_message,#dhcp_message").text("未检测到网线连接!");
        }else{
            //if (wanInfoJsonObject.mode==2&&wanInfoJsonObject.reason==22) {
            if (wanInfoJsonObject.reason==22) {
                $("#pppoe_message,#static_message,#dhcp_message").text("网络已连接!");
            }else if(wanInfoJsonObject.mode==2){
                $("#pppoe_message,#static_message,#dhcp_message").text("网络未连接!");
            }else{
                $("#pppoe_message,#static_message,#dhcp_message").text("网线已连接!");
            }
        }
    ...
    /*$("#mac_box1").text(wanConfigJsonObject.mac).show();
    $("#mac_box2").text(wanConfigJsonObject.mac).show();
    $("#mac_box3").text(wanConfigJsonObject.mac).show();*/
}
            ...
            if (wanInfoJsonObject.error == 0) {
                if (wanInfoJsonObject.connected == 0) {
                    if (wanInfoJsonObject.link == 0 && mode != 4 && mode != 5) {
                        getMsg("请插入网线！");
                        $("#" + buttonId + '_message').text('请插入网线！');
                    } else {
                        if (responseReason > 0) {
                            getMsg(errorMessage(responseReason));
                            $("#" + buttonId + '_message').text(errorMessage(responseReason));
                        } else {
                            getMsg(msgText + '超时！');
                        }
                    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error == 0) {
                ...
                if(data.time!=1){
                    $("#lan_time").val(data.time).siblings("label").hide();
                }
...
function setLanDHCP(ip, mask, start, end, enable, ipMark, dns1, dns2,time) {
    $.ajax({
        type: "POST",
        url: actionUrl + "fname=net&opt=dhcpd&function=set&start=" + start + "&end=" + end + "&mask=" + mask + "&ip=" + ip + "&time=" + time + "&enable=" + enable + "&dns1=" + dns1 + "&dns2=" + dns2,
        dataType: "JSON",
        timeout: 10000,
        success: function (data) {
            layer.closeAll();
...
function getNetMessage(sec, buttonId, msgText, mode) {
    ...
    return $("#" + buttonId + '_message').text(msgText + '，' + sec + 's');
}
