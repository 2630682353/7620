...
void function () {
	...
	$.Ajax.prototype.onCensor = function () {
		...
		if (this.responseXML && !this.timeout && s >= 200 && s < 300 || s === 304 || s === 1223) {
			...
			return txt == "0" ? true : { code: txt, des: $.lge.get(txt) ?  $.lge.get(txt, true) : Language.All[$.lge.getSe()].errdef};
		}
...
//		alert(h + "::" + px + "::" + x.style.marginBottom);
	...
	function rePlate_t(x){
		...
		if(dh > h){
			var mtt = Math.floor(Math.abs((parseInt(x.style.marginTop)/(dh - h) * (h - rh))) || 0)-13;
			if(mtt<0){
				mtt = mtt+13;
			}
			rs.style.marginTop = mtt + "px";
			
			
		}else{
			rs.style.marginTop = 0 + "px";
		}
	...
	function rePlate(dom,isSet){
		...
		if(dh > h){
			...
			var mtt = rs.style.marginTop = Math.floor(Math.abs(parseInt(x.style.marginTop) || 0)/(dh - h) * (h - rh))-13;
			if(mtt<0){
				mtt = mtt+13;
			}
			rs.style.marginTop = mtt + "px";
			
			var hh= Math.floor(rh);
			//if(hh<13){//滚动条高度最小为 13;
			//hh = 13;
			//}
			rs.style.height = hh+13 + "px";//防止滚动条变没了   当上传大量文件时，滚动条高度会很小 甚至看不到
		...
		if($.config.verify != 'none' && !isIndex){
			...
			if(this.error || v.errno != "0"){
				...
				if(iStorage && iStorage._text != storage._text){				
					//格式化SD对象统一数据,方便对比
					iStorage.sdcard  == null ? iStorage.sdcard  = "" : iStorage.sdcard  = iStorage.sdcard;
					iStorage.sdcard1 == null ? iStorage.sdcard1 = "" : iStorage.sdcard1 = iStorage.sdcard1;
					iStorage.sdcard2 == null ? iStorage.sdcard2 = "" : iStorage.sdcard2 = iStorage.sdcard2;
					iStorage.sdcard3 == null ? iStorage.sdcard3 = "" : iStorage.sdcard3 = iStorage.sdcard3;
					 storage.sdcard  == null ? storage.sdcard  = "" : storage.sdcard  = storage.sdcard;
					 storage.sdcard1 == null ? storage.sdcard1 = "" : storage.sdcard1 = storage.sdcard1;
					 storage.sdcard2 == null ? storage.sdcard2 = "" : storage.sdcard2 = storage.sdcard2;
					 storage.sdcard3 == null ? storage.sdcard3 = "" : storage.sdcard3 = storage.sdcard3;
					 //格式化SD对象的name属性统一数据,方便对比
					var isdmemory = [ iStorage.sdcard.name  == null ? "" : iStorage.sdcard.name  = iStorage.sdcard.name,
					                  iStorage.sdcard1.name == null ? "" : iStorage.sdcard1.name = iStorage.sdcard1.name,
					                  iStorage.sdcard2.name == null ? "" : iStorage.sdcard2.name = iStorage.sdcard2.name, 
					                  iStorage.sdcard3.name == null ? "" : iStorage.sdcard3.name = iStorage.sdcard3.name ];  //上次插拔信息								
					var sdmemory = [ storage.sdcard.name  == null ? "":storage.sdcard.name  = storage.sdcard.name,
					                 storage.sdcard1.name == null ? "":storage.sdcard1.name = storage.sdcard1.name,
									 storage.sdcard2.name == null ? "":storage.sdcard2.name = storage.sdcard2.name,
									 storage.sdcard3.name == null ? "":storage.sdcard3.name = storage.sdcard3.name]; // 最新插拔信息
					
					if(isdmemory[0] == "" && sdmemory[0] || isdmemory[1] == "" && sdmemory[1] ||isdmemory[2] == "" && sdmemory[2] || isdmemory[3] == "" && sdmemory[3]){
					 //两次请求数据都存在，比较两项属性，有差异，说明换卡了
					   csd = 2;
					   $.msg.alert("::HotPlug_SDCard_in");
					}else if(
					  isdmemory[0] && sdmemory[0] == "" || 
					  isdmemory[1] && sdmemory[1] == "" || 
					  isdmemory[2] && sdmemory[2] == "" || 
					  isdmemory[3] && sdmemory[3] == ""
					){
					  csd = -1;
					...
					//usb插拔信息的比较
					iStorage.usbdisk  == null ? iStorage.usbdisk  = "" : iStorage.usbdisk  = iStorage.usbdisk;
					...
					storage.usbdisk  == null ? storage.usbdisk  = "" : storage.usbdisk  = storage.usbdisk;
					...
					var iusbmemory = [ iStorage.usbdisk.name  == null ? "" : iStorage.usbdisk.name=iStorage.usbdisk.name,
									   iStorage.usbdisk1.name == null ? "" : iStorage.usbdisk1.name=iStorage.usbdisk1.name,
									   iStorage.usbdisk2.name == null ? "" : iStorage.usbdisk2.name=iStorage.usbdisk2.name,
									   iStorage.usbdisk3.name == null ? "" : iStorage.usbdisk3.name=iStorage.usbdisk3.name ];	//上次的USB插拔信息					
					var usbmemory =[ storage.usbdisk.name == null  ? "" : storage.usbdisk1.name = storage.usbdisk.name ,
									 storage.usbdisk1.name == null ? "" : storage.usbdisk1.name = storage.usbdisk1.name ,
									 storage.usbdisk2.name == null ? "" : storage.usbdisk2.name = storage.usbdisk2.name ,
									 storage.usbdisk3.name == null ? "" : storage.usbdisk3.name = storage.usbdisk3.name ];	// 这次的USB插拔信息	
					if(iusbmemory[0] == "" &&  usbmemory[0] ||iusbmemory[1] == "" &&  usbmemory[1] || iusbmemory[2] == "" &&  usbmemory[2]){
						  //两次请求数据都存在，比较两项属性，有差异，说明换卡了
							cusb = 2;
							$.msg.alert("::HotPlug_USBDevice_in");
					}else if(iusbmemory[0] && usbmemory[0] == "" || iusbmemory[1] && usbmemory[1] == "" || iusbmemory[2] && usbmemory[2] == "" ){
						cusb = -1;
...
void function () {
		...
		power.get = function(){
			...
			$.Ajax.get("/protocol.csp?fname=system&opt=i2c&function=get",function(){
				if(!this.error){
					...
					if(powIcon){
						if($.config.onBattery){
							$.config.onBattery(b,powIcon);
						}else{
							powIcon.className = b>75?"MMetro_bottom_Power":b>50?"MMetro_bottom_Power3":b>25?"MMetro_bottom_Power2":"MMetro_bottom_Power1";
						}
					...
					if(x.charge == 1){
						...
						if(powIcon){
								power.chargeInterval = setInterval(function(){
									...
									powIcon.className = 
									pc == "MMetro_bottom_Power" ? "MMetro_bottom_Power3":
									pc == "MMetro_bottom_Power3" ? "MMetro_bottom_Power2":
									pc == "MMetro_bottom_Power2" ? "MMetro_bottom_Power1":"MMetro_bottom_Power";
								},500);
							}
					}
	...
	$.systemSetMain = function(){
		if($.data.account == "guest"){
			$.vhref(isPadUI ? "user/guest.html" : "set.html");
		}else{
	...
	function padUIInit() {
		...
		if($.data.account == "guest"){
			...
			x.css("display","none").$.user.style.display = "block";
		}
