...
void function () {
	var iRoot = {}, iData;
	var v = {name: "WIFIDISK", path: "/data/", limit: "rw", dev: "sda", type: "root"};
...
//				}
    ...
    function cancelPasteup(id){
        ...
        if(v){
            ...
            if(v.isPaste){
				$.msg.openLoad();
					//decodeURIComponent(v.replace(/\/+$/,"").split("/").pop());
				    var temp = v.path.split("/");
						if(temp[temp.length - 1] == ""){
						  temp.pop();
						}
					var des = v.toPath + "" + temp[temp.length - 1];
					$.Ajax.get("/protocol.csp?fname=storage&opt=canceltrans&src="+ v.path + "&des=" + des + "&function=set",function(){
						$.msg.closeLoad();
						setTimeout(function(){
							pasteStart();
						},1000);	
...
*/
	...
	var topFuns = [
		...
		function () {
			...
			var bar = $.msg.confirm('<div class="MExplore_ipt1"><input type="text" msg_bar="nameipt" maxlength="200"></div>',function(){
				var name = bar.$nameipt.value.trim();
			if (/[\\\/\:\*\?\"\<\>\|]/g.test(name) || name.uniLength() > 200 || name.indexOf(".") == 0 || --name.length == name.indexOf(".") ) {
					$.msg.alert($.lge.get("Setting_Folder_Directory_Tip").replace("{#n}","<br /> \\ / : * ? \" < > |"),function(){
						bar.$nameipt.focus();
					});
					return false;
				}
				...
				$.msg.openLoad();
				var path = iPath + encodeURIComponent(name) + "/";
		...
		function () {
			...
			var bar = $.msg.confirm('<div class="MExplore_ipt1"><input type="text" value="' + oldName + '" msg_bar="nameipt" maxlength="200"></div>',function(){
				var newName = bar.$nameipt.value.trim();
				//if (!/^[a-zA-Z0-9\u00c0-\uFFFF\s\-_\{\}\[\]\(\)\.]+$/.test(newName) || newName.uniLength() > 200 || newName.indexOf(".") == 0 || --newName.length == newName.indexOf(".") ) {
				if (/[\\\/\:\*\?\"\<\>\|]/g.test(newName) || newName.uniLength() > 200 || newName.indexOf(".") == 0 || --newName.length == newName.indexOf(".") ) {
					$.msg.alert($.lge.get("Setting_Folder_Directory_Tip").replace("{#n}","<br /> \\ / : * ? \" < > |"));
					return false;
				}
				if (oldName == newName) {
					return;
			...
			if (this.status < 200 || this.status >= 300) {
				...
				$("NExList").innerHTML = $.forEach($.xjson(this.responseXML, "D:href"), function (href, i) {
					...
					return $.tpl.apply("li", {
						...
						type: i ? types[i] == "httpd/unix-directory" ? "directory" : name.toLowerCase().split(".").pop() : "updirectory",
	...
	$.dom.on($("b_back"), "vclick", function(){
		topFuns[8](iData[0]);
		topPath = iData[0].name;
		return false;
	});
	getDirlist(setRootHtml); //获取磁盘列表的。走ioos协议。
