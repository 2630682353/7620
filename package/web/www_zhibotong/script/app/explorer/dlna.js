...
$.Ajax.get("/dlna.csp?fname=dlna&opt=get_item_num"+ ($.data.account == "guest" ? "&Guest=1" : "") +"&function=get",function(){
	if(!this.error){
...
function getData(view,start){
	...
	$.Ajax.get("/dlna.csp?fname=dlna&opt=GetBrower&ObjectID=" + kIds[view] + "&Filter=*"+ ($.data.account == "guest" ? "&Guest=1" : "") +"&BrowseFlag=BrowseDirectChildren&SortCriteria=&RequestedCount=" + liLength + "&StartingIndex=" + start + "&function=get",function(){
//			this.error = null;
