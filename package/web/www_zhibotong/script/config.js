...
//		lge_path_us:'themes/' + theme + "/us",
		...
		hasHDD:true,   //是否HDD板子
		hasSD:false,   //是否SD板子
		hasPlug:false, //是否支持插拔信息(是否添加移除条)
		...
		hasInlayDisk:true,//是否内嵌硬盘(用于验证是否移除内置硬盘)
		hasDLNA:true,//是否有dlna服务
		dlnaMultipleDir:false,//DLNA服务多目录
		hasCheckPwd : false,//修改密码时是否验证旧密码
		hasHost:true,//是否有host功能
		hasMHz:false,//是否有频率
		hasSkip:false,//是否开启自动跳转
		hasHelpUrl:"",//外置help连接
		hasWanAccess:true,//是否允许Wan访问设备
		autoLogin:false,//是否有自动登录
		//logo图片路径如："/themes/aicodyS1/metro-logo.png，图片较大时使用，并通过css控制宽高。
		logoImg:"",
		languageName:{
			"us":"English",
			"zh_CN":"简体中文",
