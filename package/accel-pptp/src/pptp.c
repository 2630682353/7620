...
static int add_chan(struct pppox_sock *sock)
{
	...
	spin_lock(&chan_lock);
	get_random_bytes(&call_id, 1);
	if (call_id < 0 || call_id > MAX_CALLID || test_bit(call_id, callid_bitmap)) {
...
static int pptp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,36)
	rt = ip_route_output_ports(sock_net(sk), (struct flowi4 *)&fl, NULL,
	...
	fl.nl_u.ip4_u.daddr = opt->dst_addr.sin_addr.s_addr;
	fl.nl_u.ip4_u.saddr = opt->src_addr.sin_addr.s_addr;
...
static int pptp_rcv_core(struct sock *sk,struct sk_buff *skb)
{
	...
	if (PPTP_GRE_IS_A(header->ver)) {
		...
		u32 ack = (PPTP_GRE_IS_S(header->flags))? header->ack:header->seq;
		ack = ntohl( ack);
		if (ack > opt->ack_recv)
			opt->ack_recv = ack;
...
#ifdef DEBUG
		if (log_level >= 1)
			printk(KERN_INFO"PPTP: discarding truncated packet "
...
#ifdef DEBUG
			if ( log_level >= 1)
				printk(KERN_INFO"PPTP[%i]: allowing old LCP "
								"Echo packet %d (expecting %d)\n",
...
#ifdef DEBUG
		if ( log_level >= 1)
			printk(KERN_INFO"PPTP[%i]: discarding duplicate or "
							"old packet %d (expecting %d)\n",
							opt->src_addr.call_id,
							seq, opt->seq_recv + 1);
#endif
	} else { 
...
#ifdef DEBUG
		if ( log_level >= 3 && opt->seq_sent<=log_packets)
			printk(KERN_INFO"PPTP[%i]: accepting packet %d size=%i "
...
#ifdef DEBUG
		if (log_level >= 1)
			printk(KERN_INFO"PPTP: Discarding packet from "
							"unknown call_id %i\n",
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,36)
	rt = ip_route_output_ports(sock_net(sk), (struct flowi4 *)&fl, sk,
				   opt->dst_addr.sin_addr.s_addr,
				   opt->src_addr.sin_addr.s_addr,
				   0, 0,
