...
#ifdef HAVE_HOST_TYPE
static void dhcp_ops_send(struct host_dhcp_trait *ops)
{
	struct nlk_dhcp_msg nlk;

	memset(&nlk, 0, sizeof(nlk));
	nlk.comm.gid = NLKMSG_GRP_SYS;
	nlk.comm.mid = SYS_GRP_MID_DHCP;
	memcpy(nlk.dhcp, ops, sizeof(nlk.dhcp));
	nlk_event_send(NLKMSG_GRP_SYS, &nlk, sizeof(nlk));
}

static int dhcp_get_ops(unsigned char *haddr, unsigned char *option)
{
	struct host_dhcp_trait ops[5];
	unsigned char *src = option + 4;
	unsigned char len = 0, i = 0;
	unsigned char cplen = 0, oplen = 0;

	memset(ops, 0, sizeof(ops));
	oplen = sizeof(ops[0].op) - 1;
	while ((*src != 0) && (*src != 255)) {
		if (((unsigned int)(src - option)\
				+ src[1] + 2) > 299) {
			return -1;
		}
		ops[0].op[i] = *src;
		...
		len = *src;
		src++;
		cplen = len > oplen ? oplen : len;
		switch (ops[0].op[i]) {
		case 53:
			ops[0].type = *src;
			break;
		case 55:
			memcpy(ops[1].op, src, cplen);
			ops[1].op[cplen] = 255;
			break;
		case 60:
			memcpy(ops[2].op, src, cplen);
			break;
		case 12:
			memcpy(ops[3].op, src, cplen);
			break;
		default:
			break;
		}
	...
	ops[0].op[i] = 255;
	memcpy(ops[4].op, haddr, oplen > DHCP_CHADDR_MAX ? DHCP_CHADDR_MAX : oplen);
	if (ops[0].type == DHCPDISCOVER) {
		ops[0].type = DIS_OPS;
		ops[1].type = DIS_OP55;
	} else if (ops[0].type == DHCPREQUEST) {
		ops[0].type = REQ_OPS;
		ops[1].type = REQ_OP55;
	} else {
		return -1;
		  ...
		  size_t sz, time_t now, int unicast_dest, int *is_inform, int pxe, struct in_addr fallback)
{
  ...
  if ((opt = option_find(mess, sz, OPTION_MESSAGE_TYPE, 1)))
	...
	if ((mess_type == DHCPDISCOVER) || (mess_type == DHCPREQUEST)) {
		dhcp_get_ops(mess->chaddr, mess->options);
	}
#endif
