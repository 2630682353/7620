...
#include <sys/ipc.h>
#include <sys/shm.h>
int addNewServer(int option, char *arg, int servers_only)
{
    struct server *serv, *newlist = NULL;
    ...
    newlist->flags |= SERV_FROM_JS; 
    serv->next = daemon->servers;
    daemon->servers = newlist;
    return 0;
}

void replace_dnsserver(const char* newaddr)
{
    struct server *old;
    old=daemon->servers;
    if(old->flags&SERV_FROM_JS)
    {
        daemon->servers=old->next;
        free(old);
        addNewServer('S', newaddr, 0);
    }
    else
    {
        addNewServer('S', newaddr, 0);
...
void dump_servers()
{
    ...
    while(serv)
    {
       printf("serv flag:%x,addr:%x\n",serv->flags,serv->addr.in.sin_addr.s_addr);
        serv=serv->next;
    }
}
char dns_servaddr[IP_ADDR_STR_LEN]={0};
void dnsserver_update()
{
    int shmid=0;
    char * shmem=NULL;
    char tmp_server[IP_ADDR_STR_LEN]={0};
    int myDnsStatus=0;
    shmid=shmget(CONFIG_UPDATE_MEM_KEY,IP_ADDR_STR_LEN+sizeof(int),IPC_CREAT|0777);
   if(shmid>=0)
    {
        shmem=(char*)shmat(shmid,NULL,0);
        if(shmem)
        {
            memcpy(&myDnsStatus,shmem, sizeof(int));
            memcpy(tmp_server,shmem+sizeof(int),IP_ADDR_STR_LEN);
            //my_syslog(LOG_WARNING,_("%s:%d %d\n"),tmp_server,myDnsStatus,shmid);
            //printf("----%s:%d----\n",tmp_server,myDnsStatus);
            shmdt(shmem);
            if(!strlen(tmp_server))
            {
                printf("dns server not configed\n");
                return;
            }
            if(strcmp(dns_servaddr,tmp_server))
            {
                /*need update*/
                memset(dns_servaddr,0,sizeof(dns_servaddr));
                memcpy(dns_servaddr,tmp_server,strlen(tmp_server));
                //printf("before replace\n");
                //dump_servers();
                replace_dnsserver(dns_servaddr);
    ...
    {
        my_syslog(LOG_WARNING, _("create error: %d %d\n"),shmid, errno);
...
int getMyDnsStatus(void)
{
    int g_shmid=0;
    char *g_shmem;

    int myDnsStatus=0;
//CONFIG_UPDATE_MEM_KEY
    g_shmid=shmget(CONFIG_UPDATE_MEM_KEY,IP_ADDR_STR_LEN+sizeof(int),IPC_CREAT|0777);
    if(g_shmid>=0)
    {
        g_shmem=(char*)shmat(g_shmid,NULL,0);
        if(g_shmem)
        {
            memcpy(&myDnsStatus, g_shmem, sizeof(int));
                        
            shmdt(g_shmem);
        }
    }
    //my_syslog(LOG_WARNING, _("status: %d %d %d\n"), myDnsStatus,g_shmid,errno);
