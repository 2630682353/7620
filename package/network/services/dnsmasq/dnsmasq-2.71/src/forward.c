...
//veryci
extern char dns_servaddr[IP_ADDR_STR_LEN];
...
//veryci
  ...
  if (!flags && forward)
	...
	{ 
        //veryci add
#if 1
		myDnsStatus = getMyDnsStatus();
        struct all_addr *paddr1 = (struct all_addr *)&start->addr.in.sin_addr;
        if((0 == myDnsStatus) && (paddr1->addr.addr4.s_addr ==  inet_addr(dns_servaddr)))
		{
            forward->forwardall = 1;
            if (!(start = start->next)){
 	            break;
...
//veryci
		  ...
		  if (!forward->forwardall) 
		    break;
#else
                  struct all_addr *paddr = (struct all_addr *)&start->addr.in.sin_addr;
                  myDnsStatus = getMyDnsStatus();
                  if((1 == myDnsStatus) && (paddr->addr.addr4.s_addr == (inet_addr(dns_servaddr))) ){
                      forward->forwardall = 0;
                                      break;
                  }else{
                      forward->forwardall = 1;
...
//veryci
  ...
  if ((RCODE(header) == SERVFAIL || RCODE(header) == REFUSED) &&
      ...
      forward->forwardall == 0)
#else
   if ((RCODE(header) == REFUSED ) && (forward->forwardall == 0))
#endif
    /* for broken servers, attempt to send to another one. */
    {
      ...
      size_t plen;
      int is_sign;
      my_syslog(LOG_WARNING, _("reply refuse"));//hack
