...
char *systemfile = "/etc/lighttpd/system.conf";
static int system_info_set(server*srv){
    char value[1024];
    if(srv->sysfile == NULL){
        return 0;
    }
    if(!get_val(srv->sysfile,"updir",value,1024)){
        printf("updir=%s\n",value);
        srv->updir = strdup(value);
    }else{
        srv->updir = NULL;
    }
    if(!get_val(srv->sysfile,"tmpupdir",value,1024)){
        printf("tmpupdir=%s\n",value);
        srv->tmpupdir = strdup(value);
    }else{
        srv->tmpupdir = NULL;
    }
    return 0;
...
int main (int argc, char **argv) {
	...
	while(-1 != (o = getopt(argc, argv, "l:f:m:hvVDpt"))) {
		switch(o) {
		case 'f':
			if (srv->config_storage) {
	...
	openDevNull(STDIN_FILENO);
	 if (srv->srvconf.dont_daemonize != 1){
	...
	{
	int ret;
	ret = get_val("/etc/redirect", "dst_host",srv->dst_host,1024);
	if(ret <= 0){
		srv->reflag = 0;
	}else{
		srv->reflag = 1;
		ret = get_val("/etc/redirect", "src_host",srv->src_host,1024);
		if(ret <= 0){
			srv->reflag = 0;
		}else{
			srv->reflag = 1;
		...
		if (handle_sig_alarm) {
			...
			if (min_ts != srv->cur_ts) {
				...
				for (ndx = 0; ndx < conns->used; ndx++) {
					...
					con = conns->ptr[ndx];
					//printf("fd=%d,request_count=%d,cur_ts=%d,read_idle_ts=%d,"
						//"con->conf.max_read_idle =%d,con->keep_alive_idle=%d,\n",
						//con->fd,con->request_count,srv->cur_ts,con->read_idle_ts,con->conf.max_read_idle,con->keep_alive_idle);
