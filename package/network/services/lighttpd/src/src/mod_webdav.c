...
static char * uploadir_char_clear( char* src)
{
    while(*src == '/')
    {
        src++;
...
URIHANDLER_FUNC(mod_webdav_subrequest_handler) {
	...
	case HTTP_METHOD_COPY: {
		...
		if(srv->updir){
			...
			buffer_copy_string_len(p->physical.path, CONST_STR_LEN("/"));
		}
		printf("dstdir1=%s\n",p->physical.path->ptr);
		//buffer_copy_string_buffer(p->physical.path, p->physical.doc_root);
...
#if 1
        strcpy(webdav_physical,p->physical.path->ptr);
		printf("webdav_physical=%s\n",webdav_physical);
		if (overwrite == 0) {
			if (0 == stat(webdav_physical, &st)) {
				printf("testn1webdav\n");
				con->http_status = 412;
				return HANDLER_FINISHED;
            }else
            {
                int len = strlen(webdav_physical);
                if(webdav_physical[len -1] == '/')
                {
                    char buf[1024];
                    strcpy(buf, webdav_physical);
                    buf[len -1] = '\0';
                    if ( 0 == stat(buf,&st)){
				        con->http_status = 412;
						printf("testn2webdav\n");
		...
		printf("testn3webdav\n");
			if (con->request.http_method == HTTP_METHOD_MOVE){
                char src[1024];
                char *p1;
                strcpy(src,con->physical.path->ptr);
                p1 = src;
                p1 = uploadir_char_clear(p1);
                p1 = strchr(p1,'/');
                p1 = uploadir_char_clear(p1);
                p1 = strchr(p1,'/');
                p1 = uploadir_char_clear(p1);
		...
		printf("src=%s\n",src);
		printf("dest=%s\n",webdav_physical);
                if(strncmp(webdav_physical,src,strlen(src)) == 0)
                {
            		int status;
        		if ( 0 == stat(webdav_physical,&st)){
            			if(S_ISDIR(st.st_mode)){
                			if((status =webdav_mv_delete_dir(webdav_physical)) != 0){
			        		con->http_status = status; /* we will create a new one */
			        		con->file_finished = 1;
			        		return HANDLER_FINISHED;
			    		}

					if (-1 == rmdir(webdav_physical)) {
                    				switch(errno) {
                        				case ENOENT:
                            					con->http_status = 404;
                            					break;
			...
			printf("go to rename,test1\n");
			printf("rname %s %s\n",con->physical.path->ptr,webdav_physical);
			if(rename(con->physical.path->ptr,webdav_physical) == -1){
				con->http_status = 201; /* we will create a new one */
				con->file_finished = 1;
				printf("go to rename,test2\n");
				return HANDLER_FINISHED;
			}else{
				con->http_status = 201;
				con->file_finished = 1;
				printf("go to rename,test3\n");
				return HANDLER_FINISHED;
			}
                }
		}
		{	
                	...
                	char src[1024];
			strcpy(src,con->physical.path->ptr);
			strcpy(dest,webdav_physical);
                	//cmd_print(src,con->physical.path->ptr);
                	//cmd_print(dest,webdav_physical);
			printf("%s %s %s %s\n","filecp","cp",src, dest);
			switch (fork()) {
				case -1:
					con->http_status = 501; /* we will create a new one */
					con->file_finished = 1;
					return HANDLER_FINISHED;
				case 0:
					break;
				default:
					//close(con->fd);
					con->http_status = 200; /* we will create a new one */
					con->file_finished = 1;
					return HANDLER_FINISHED;
					//return HANDLER_ERROR;
			}
			dup2(con->fd,3);
			if (con->request.http_method == HTTP_METHOD_MOVE){
        			execl("/bin/filecp","mv",src, dest, (char *) 0); 
			}else{
        			execl("/bin/filecp","cp",src, dest, (char *) 0); 
			}
			exit(0);
		}
#endif
