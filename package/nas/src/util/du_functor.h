#ifndef __DU_FUNCTOR_H
#define __DU_FUNCTOR_H
 ...
 *  }
 *
 * DU_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(TestFunction3);
 * string s3("s3");
 * cmd3(s3, 10);
 * C函数调用用DU_FunctorWrapper封装:*
 *  DU_FunctorWrapper<DU_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, s3, 10);
 *  fwrapper3();
 * 说明:
 * void : 函数的返回值
 * C++函数对象调用:
 ...
 *  TestFunctor f;
 *
 *  DU_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(f);
 *
 *  string s3("s3");
 *
 *  cmd3(s3, 10);
 *
 * C++函数对象调用用DU_FunctorWrapper封装:
 *
 *  DU_FunctorWrapper<DU_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, s3, 10);
 *
 *  fwrapper3();
 ...
 *     }
 *  }
 *  DU_Functor<void, TL::TLMaker<const string&, int>::Result> cmd3(&tm, &TestMember::mem3);
 * cmd3("a", 33);
 * 指向类成员函数的调用用DU_FunctorWrapper封装:
 *  DU_FunctorWrapper<DU_Functor<void, TL::TLMaker<const string&, int>::Result> > fwrapper3(cmd3, "a", 10);
 *  fwrapper3();
 * 注意所有的DU_FunctorWrapper的调用, 在具体调用的时候是不带参数的, 参数在构造的时候已经传进去了
 ///////////////////////////////////////////////////////////////////////////////////////////////////
 *
 * 对于C风格的函数调用, 同名称的函数通常只有一个, 如果有多个相同名称的函数
 *不同参数类型的函数, 则在构造DU_Functor的时候需要强制类型转换, 如下:
 *  void TestFunction1()
 *  {
 ...
 * }
 * typedef void (*TpFunc)();
 * DU_Functor<void> cmd1(static_cast<TpFunc>(TestFunction1));
 * cmd1();
 * typedef void (*TpFunc1)(char);
 * DU_Functor<void, TL::TLMaker<char>::Result>
 ...
 *  DU_Functor<void> cmd1(&tm, static_cast<TpMem>(&TestMember::mem1));
 *  cmd1();
 *
 *  typedef void (TestMember::*TpMem1)(char);
 
 * DU_Functor<void, TL::TLMaker<char>::Result> cmd11(&tm,
 
 * static_cast<TpMem1>(&TestMember::mem1)); cmd11('c');
 *
 * 具体示例请参见test/test_du_functor.cpp.
 */
...
public:
    ...
    virtual DU_FunctorImp* clone() const = 0;
    virtual ~DU_FunctorImp() {};
};

template<typename R>
class DU_FunctorImp<R, TL::EmptyType>
{
public:
    virtual R operator()() = 0;
    virtual DU_FunctorImp* clone() const = 0;
...
class DU_FunctorImp<R, TL::TYPELIST_1(P1)>
{
public:
...
class DU_FunctorImp<R, TL::TYPELIST_2(P1, P2)>
{
public:
...
class DU_FunctorImp<R, TL::TYPELIST_3(P1, P2, P3)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
...
class DU_FunctorImp<R, TL::TYPELIST_4(P1, P2, P3, P4)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_5(P1, P2, P3, P4, P5)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_6(P1, P2, P3, P4, P5, P6)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_7(P1, P2, P3, P4, P5, P6, P7)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_8(P1, P2, P3, P4, P5, P6, P7, P8)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_9(P1, P2, P3, P4, P5, P6, P7, P8, P9)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorImp<R, TL::TYPELIST_10(P1, P2, P3, P4, P5, P6, P7, P8, P9, P10)>
{
public:
    virtual R operator()(typename TL::TypeTraits<P1>::ReferenceType p1,
                         typename TL::TypeTraits<P2>::ReferenceType p2,
...
class DU_FunctorHandler : public DU_FunctorImp<typename ParentFunctor::ResultType, typename ParentFunctor::ParamList>
{
public:
    typedef typename ParentFunctor::ResultType ResultType;

    DU_FunctorHandler(Fun fun) : _fun(fun)
    {

    }

    DU_FunctorHandler *clone() const
    {
        return new DU_FunctorHandler(*this);
    }
 ...
 * @brief 封装对成员函数的调用
 */
class DU_MemFunHandler
	: public DU_FunctorImp<typename ParentFunctor::ResultType, typename ParentFunctor::ParamList>
{
public:
	typedef typename ParentFunctor::ResultType ResultType;

	DU_MemFunHandler(const PointerToObj &pObj, PointerToMemFun pMemFn)
		: _pObj(pObj), _pMemFn(pMemFn)
	{
	}

	DU_MemFunHandler* clone() const
	{
		return new DU_MemFunHandler(*this);
	}
...
public:
    ...
    /**定义封装类型*/
    typedef DU_FunctorWrapper<DU_Functor<R, TList> > wrapper_type;
...
public:
    ...
    DU_Functor(const DU_Functor &functor)
    : _spImpl(functor._spImpl->clone())
    {
    }

    DU_Functor& operator=(const DU_Functor &functor)
    {
        if(this != &functor)
        {
            _spImpl = std::shared_ptr<Impl>(functor._spImpl->clone());
        }
    ...
    template<class Fun>
    DU_Functor(Fun fun)
	: _spImpl(new DU_FunctorHandler<DU_Functor, Fun>(fun))
    {
    }

	template<typename PointerToObj, typename PointerToMemFun>
	DU_Functor(const PointerToObj &pObj, PointerToMemFun pMemFn)
	: _spImpl(new DU_MemFunHandler<DU_Functor, PointerToObj, PointerToMemFun>(pObj, pMemFn))
	{
	}
...
private:
    template<class ParentFunctor>
    friend class DU_FunctorWrapper;

    typedef DU_FunctorImp<R, TList> Impl;

//    std::auto_ptr<Impl>     _spImpl;
    std::shared_ptr<Impl>       _spImpl;
};

/**
 * @brief wapper基类
 */
class DU_FunctorWrapperInterface
{
public:
    virtual void operator()() = 0;
    virtual ~DU_FunctorWrapperInterface(){}
};

template<class ParentFunctor>

/**
 * @brief 对DU_Functor进行封装, 可以让DU_Functor做到事后被调用
 */
class DU_FunctorWrapper : public DU_FunctorWrapperInterface
{
public:
     ...
     * @param fw
     */
    DU_FunctorWrapper(const DU_FunctorWrapper &fw)
     ...
     * @param tf
     */
    DU_FunctorWrapper(ParentFunctor &tf)
...
protected:
     ...
     * @param fw
     *
     * @return DU_FunctorWrapper&
     */
    DU_FunctorWrapper& operator=(const DU_FunctorWrapper &fw);
