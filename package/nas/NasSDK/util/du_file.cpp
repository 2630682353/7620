...
bool DU_File::isAbsolute(const string &sFullFileName)
{
...
bool DU_File::isFileExist(const string &sFullFileName, mode_t iFileType)
{
    struct stat f_stat;
...
bool DU_File::isFileExistEx(const string &sFullFileName, mode_t iFileType)
{
    struct stat f_stat;
...
bool DU_File::makeDir(const string &sDirectoryPath, mode_t iFlag)
{
...
bool DU_File::makeDirRecursive(const string &sDirectoryPath, mode_t iFlag)
{
...
int DU_File::setExecutable(const string &sFullFileName, bool canExecutable)
{
    struct stat f_stat;
...
bool DU_File::canExecutable(const string &sFullFileName)
{
    struct stat f_stat;
...
int DU_File::removeFile(const string &sFullFileName, bool bRecursive)
{
...
string DU_File::simplifyDirectory(const string& path)
{
    string result = path;
...
void DU_File::save2file(const string &sFullFileName, const string &sFileData)
{
...
int DU_File::save2file(const string &sFullFileName, const char *sFileData, size_t length)
{
...
string DU_File::getExePath()
{
    ...
    if ( count < 0 )
    {
        throw DU_File_Exception("[DU_File::getExePath] could not get exe path error", errno);
    }
...
string DU_File::extractFilePath(const string &sFullFileName)
{
    if(sFullFileName.length() <= 0)
    {
        return "./";
...
string DU_File::extractFileExt(const string &sFullFileName)
{
    string::size_type pos;
...
string DU_File::excludeFileExt(const string &sFullFileName)
{
    string::size_type pos;
    if ((pos = sFullFileName.rfind('.')) == string::npos)
    {
...
string DU_File::replaceFileExt(const string &sFullFileName, const string &sExt)
{
    return excludeFileExt(sFullFileName) + "." + sExt;
}

string DU_File::extractUrlFilePath(const string &sUrl)
{
    string sLowerUrl = DU_Common::lower(sUrl);
...
size_t DU_File::scanDir(const string &sFilePath, vector<string> &vtMatchFiles, FILE_SELECT f, int iMaxSize )
{
    vtMatchFiles.clear();
...
void DU_File::copyFile(const string &sExistFile, const string &sNewFile,bool bRemove)
{
    if(DU_File::isFileExist(sExistFile, S_IFDIR))
    {
        DU_File::makeDir(sNewFile);
        vector<string> tf;
        DU_File::scanDir(sExistFile,tf, 0, 0);
        for(size_t i = 0; i <tf.size(); i++)
        {
            if(tf[i] == "." || tf[i] == "..")
            continue;
    ...
    else
    {
        ...
        if(!fin)
        {
            throw DU_File_Exception("[DU_File::copyFile] error: "+sExistFile, errno);
        }
        std::ofstream fout(sNewFile.c_str());
        if(!fout )
        {
             throw DU_File_Exception("[DU_File::copyFile] error: "+sNewFile, errno);
        }
        struct stat f_stat;
...
time_t DU_File::getModifyTime(const std::string& path)
{
    struct stat f_stat;
