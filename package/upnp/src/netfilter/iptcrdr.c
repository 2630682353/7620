...
static void
add_redirect_desc(unsigned short eport, int proto, const char *iaddr, unsigned short iport, const char *desc)
{
	struct mu_upnpd upnp;

	if (!iaddr || !desc)
		return;
	memset(&upnp, 0x0, sizeof(upnp));
	upnp.action = NLK_ACTION_ADD;
	upnp.upnp.in.port  = iport;
	upnp.upnp.out.port = eport;
	upnp.upnp.proto = proto;
	inet_aton(iaddr, &upnp.upnp.in.ip);
	strncpy(upnp.upnp.desc, desc, sizeof(upnp.upnp.desc));
	mu_msg(UPNPD_RULE_ADD, &upnp, sizeof(struct mu_upnpd), NULL, 0);
...
/*	if(desc)
	{*/
		...
		if(p)
		{
			...
			p->proto = (short)proto;
			p->iport = iport;
			inet_aton(iaddr, &p->iaddr);
			if(desc)
				memcpy(p->str, desc, l);
			else
				memcpy(p->str, "upnp", 4);
			rdr_desc_list = p;
			upnp_info_2_mu(p, NLK_ACTION_ADD);
		}
/*	}*/
	#endif
}
...
del_redirect_desc(unsigned short eport, int proto)
{
	struct mu_upnpd upnp;
	memset(&upnp, 0x0, sizeof(upnp));
	upnp.action = NLK_ACTION_DEL;
	upnp.upnp.out.port = eport;
	upnp.upnp.proto = proto;
	mu_msg(UPNPD_RULE_DEL, &upnp, sizeof(struct mu_upnpd), NULL, 0);
	#if 0
...
int get_redirect_desc(unsigned short eport, int proto, unsigned short *iport,
	char *iaddr, int iaddrlen, char *desc, int desclen)
{
	struct upnpd info;
	struct mu_upnpd upnp;

	if (!iaddr || (iaddrlen == 0) || !iport)
		return -1;
	memset(&upnp, 0x0, sizeof(upnp));
	upnp.action = NLK_ACTION_DUMP;
	upnp.upnp.out.port = eport;
	upnp.upnp.proto = proto;
	if (mu_msg(UPNPD_RULE_DUMP, &upnp, sizeof(struct mu_upnpd), &info, sizeof(struct upnpd)))
		return -1;

	*iport = info.in.port;
	strncpy(iaddr, inet_ntoa(info.in.ip), iaddrlen);
	if (desc && desclen > 0)
		strncpy(desc, info.desc, desclen);
	return 0;	
...
{	
	...
	if(r >= 0)
	#endif
	add_redirect_desc(eport, proto, iaddr, iport, desc);
	return 0;
}
...
{
	#if 0
	return add_filter_rules(proto, iaddr, iport);
	#endif
	return 0;
}
 ...
 * */
int
add_filter_rules(int proto, const char * iaddr, unsigned short iport)
{
	int r = 0;
	struct ipt_entry * e;
	struct ipt_entry_match *match = NULL;
	...
	r = iptc_init_verify_and_append("filter", miniupnpd_forward_chain, e, "add_filter_rules()");
	free(target);
	free(match);
	free(e);
	return r;
