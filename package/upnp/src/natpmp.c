	...
	case 2:	/* TCP port mapping request */
		{
			...
			if(lifetime == 0) {
			   ...
			   || !check_upnp_rule_against_permissions(upnppermlist, num_upnpperm, eport, senderaddr.sin_addr, iport)) {
				...
				r = get_redirect_desc(eport, proto, &iport_old, iaddr_old, sizeof(iaddr_old), NULL, 0);
				#if 0
...
int CleanExpiredNATPMP()
{
	...
	if (get_redirect_desc(nextnatpmptoclean_eport, nextnatpmptoclean_proto,
						&iport, iaddr, sizeof(iaddr), desc, sizeof(desc)))
