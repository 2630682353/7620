...
joylink_proc_lan_script_ctrl(uint8_t *src, struct sockaddr_in *sin_recv, socklen_t addrlen)
{
    ...
    char * response = NULL;
    char *data2 = NULL;
    log_debug("Control from lan:%s:len:%d\n",
            src + 12, strlen((char*)(src + 12)));

    if(response = joylink_dev_script_ctrl((const char *)src, &ctr, 0) == NULL){
        return;
    }
    ret = joylink_packet_script_ctrl_rsp(&data2, &ctr, response);
    free(response);
    //log_info("rsp data:%s:len:%d", data2 + 12, ret);
	...
	if(data2 != NULL){
			...
			if(_g_pdev->payload_total < JL_MAX_PAYLOAD_LEN){
				len = joylink_encrypt_lan_basic(_g_pdev->send_buff, JL_MAX_PACKET_LEN, 
						ET_ACCESSKEYAES, PT_SERVERCONTROL, (uint8_t*)_g_pdev->jlp.localkey, 
						(uint8_t*)data2, ret);
				...
				len = joylink_encrypt_lan_basic((uint8_t*)pout, max_out, 
						ET_ACCESSKEYAES, PT_SERVERCONTROL, (uint8_t*)_g_pdev->jlp.localkey, 
						(uint8_t*)data2, ret);	
				printf("lan big data encrypt  len:%d\n", len);
			}		
