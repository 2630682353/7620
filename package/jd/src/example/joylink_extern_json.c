...
char * joylink_dev_parse_ctrl(const char * pMsg)
{
	int ret = -1;
	if(NULL == pMsg){
		return ret;
	...
	WIFIInfo_t *pw = NULL;
	cJSON *response, *obj, *cgi_response, *stream_array, *item;
	response = cJSON_CreateObject();
	cgi_response = cJSON_CreateObject();
	...
	if(NULL != pStreams){
		...
		pSub = cJSON_GetArrayItem(pStreams,0);

		cJSON *pId = cJSON_GetObjectItem(pSub, "stream_id");
		
		cJSON *pV = cJSON_GetObjectItem(pSub, "current_value");
		stream_array = cJSON_CreateArray();
	

		if (memcmp(pV->valuestring,"deviceid=",9) == 0) {
			char deviceid[10] = {0};
			memcpy(deviceid, (pV->valuestring), strlen(pV->valuestring) - 9);
		}
		else if (memcmp(pV->valuestring,"split=",6) == 0) {
			item = cJSON_CreateObject();
			printf("%s\n", (pV->valuestring)+6);
			int current = atoi((pV->valuestring)+6);
			printf("current:%d\n",current);
			obj = cJSON_CreateString(pId->valuestring);
			cJSON_AddItemToObject(item, "stream_id", obj);
			obj = cJSON_CreateString(pV->valuestring);
			cJSON_AddItemToObject(item, "current_value", obj);
			cJSON_AddItemToArray(stream_array,item);
			char temp[CUT_SIZE + 12] = {0};
			
			if (current * CUT_SIZE >= strlen(cgi_res)) {
				memcpy(temp,pV->valuestring,strlen(pV->valuestring));
				memcpy(temp+strlen(pV->valuestring),"@@",2);
				strcpy(temp+strlen(pV->valuestring)+2, cgi_res + (current -1)*CUT_SIZE);
		
			} else {
			
				memcpy(temp,pV->valuestring,strlen(pV->valuestring));
				memcpy(temp+strlen(pV->valuestring),"@@",2);
				memcpy(temp + strlen(pV->valuestring)+2, cgi_res+(current-1)*CUT_SIZE,CUT_SIZE);
					
			}
			item = cJSON_CreateObject();
			obj = cJSON_CreateString("result");
			cJSON_AddItemToObject(item, "stream_id", obj);
			printf("temp:%s\n",temp);
			obj = cJSON_CreateString(temp);
			cJSON_AddItemToObject(item, "current_value", obj);
			cJSON_AddItemToArray(stream_array,item);
			
		} else {	
			int str_size = strlen("http://127.0.0.1/protocol.csp?")+strlen(pV->valuestring)+2;
			char *url_str = malloc(str_size);
			memset(url_str, 0, str_size);		
			snprintf(url_str, str_size,"http://127.0.0.1/protocol.csp?%s", pV->valuestring);
			
			char *cgi_result = http_get(url_str);		
			printf("url is :%s\n", url_str);
			free(url_str);
			VS_StrTrim(cgi_result);
			int cgi_size = strlen(cgi_result);
			item = cJSON_CreateObject();
			obj = cJSON_CreateString(pId->valuestring);
			cJSON_AddItemToObject(item, "stream_id", obj);
			obj = cJSON_CreateString(pV->valuestring);
			cJSON_AddItemToObject(item, "current_value", obj);
			cJSON_AddItemToArray(stream_array,item);
			if (cgi_size > CUT_SIZE) {
				if (cgi_res != NULL) {
					free(cgi_res);
				}
				cgi_res = malloc(cgi_size+1);
				memset(cgi_res, 0,cgi_size+1);
				memcpy(cgi_res, cgi_result, cgi_size);
				
				int zcps = cgi_size/800;
				if (cgi_size%800)
					zcps++;
				char temp[CUT_SIZE + 12] = {0};
				printf("cgi_size:%d size=%d\n",cgi_size,zcps);
				snprintf(temp,11,"split=%d@@",zcps);
				//memcpy(temp,"split=1@@",9);
				memcpy(temp + strlen(temp), cgi_res,CUT_SIZE);
				item = cJSON_CreateObject();
				obj = cJSON_CreateString("result");
				cJSON_AddItemToObject(item, "stream_id", obj);
				obj = cJSON_CreateString(temp);
				...
				obj = cJSON_CreateString("result");
				cJSON_AddItemToObject(item, "stream_id", obj);
				obj = cJSON_CreateString(cgi_result);
				free(cgi_result);
		...
		obj = cJSON_CreateNumber(0);
		cJSON_AddItemToObject(response, "code", obj);
		cJSON_AddItemToObject(response, "streams", stream_array);
		//obj = cJSON_CreateString("done");
		//cJSON_AddItemToObject(response, "msg", obj);	
		out_str= cJSON_Print(response);
		
		//printf("this is cgi return:%s\n",out_str);
            }//for
	cJSON_Delete(pJson);
