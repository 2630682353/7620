...
int ExecuteUpdate(PHGlobal *phglobal)
{
	...
	int zc_fd;
	if ((zc_fd = open("/tmp/ddns_temp", O_RDWR)) >= 0) {
				write(zc_fd,"1",1);
				close(zc_fd);
	}
	memset(buffer, 0, 128);
	...
	if (strcmp(buffer,"250")!=0 && strcmp(buffer,"536")!=0)
	{
		...
		if (strstr(buffer + 4, "Busy.") != NULL) {
			if ((zc_fd = open("/etc/ddns_temp", O_RDWR)) >= 0) {
				lseek(zc_fd, 2, SEEK_SET);
				write(zc_fd,"2",1);
				close(zc_fd);
			}
			return errorAuthBusy;
		}
		if ((zc_fd = open("/tmp/ddns_temp", O_RDWR)) >= 0) {
			lseek(zc_fd, 2, SEEK_SET);
			write(zc_fd,"0",1);
	...
	if (strcmp(buffer,"250") == 0) //get user type level, 0(free),1(pro),2(biz)
	{
		...
		if ((zc_fd = open("/tmp/ddns_temp", O_RDWR)) >= 0) {
			lseek(zc_fd, 1, SEEK_SET);
				write(zc_fd,pos0 + 1,1);
				write(zc_fd,"11",2);
				close(zc_fd);
		}
	}
...
int phddns_step(PHGlobal *phglobal)
{
	...
	if (phglobal->bNeed_connect)
	{
		...
		if (ret == okDomainsRegistered) 
		{
			...
			if ((zc_fd = open("/tmp/ddns_temp", O_RDWR)) >= 0) {
				lseek(zc_fd, 4, SEEK_SET);
				int i=0;
				while(phglobal->szActiveDomains[i][0] != '.') {
					write(zc_fd,phglobal->szActiveDomains[i],strlen(phglobal->szActiveDomains[i]));
					write(zc_fd," ",1);
					i++;
				}
				close(zc_fd);
