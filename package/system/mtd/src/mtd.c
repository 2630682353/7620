...
static void mtd_ok(void)
{
	...
	uint32_t rand;
	uint32_t now;
	char pwd[16] = {0xc0, 0x11, 0x03, 0xcc,
	       	 	0x5c, 0x77, 0xde, 0x75,
			0x80, 0x55, 0x00, 0x7c};
...
{	
	...
	unsigned char pwd[16] = {0xdd, 0x6a, 0x78, 0x66, 0x35, 0x32, 0x97,
				 0xfc, 0x78, 0x99, 0x12, 0x23, 0x34, 0x54,
				  0x90, 0x87};
	
	
	MD5_Init(&md5);

	MD5_Update(&md5, pwd, sizeof(pwd));

	read(imagefd, &header, sizeof(header));
	/*
	totalnum = (unsigned int)(buf[12]<<24)+(unsigned int)(buf[13]<<16)
				+(unsigned int)(buf[14]<<8)
				+(unsigned int)(buf[15]);
	*/
	totalnum = htonl(header.ih_size);

	if (lseek(imagefd, sizeof(header), SEEK_SET) == -1) {
		fprintf(stderr, "Can't lseek!\n");
		return -1;
	}
	
	while ((n = read(imagefd, buf, 4096)) > 0) {
		totalnum = totalnum - n;
		if (totalnum < 4096) {
			MD5_Update(&md5, buf, n);
			n = read(imagefd, buf, totalnum);
			MD5_Update(&md5, buf, n);
			break;
		}
		MD5_Update(&md5, buf, n);
	...
	if (memcmp(res, header.ih_name + 16, sizeof(res))) {
		return -1;
	}

	if (lseek(imagefd, 0, SEEK_SET) == -1) {
		fprintf(stderr, "Can't lseek!\n");
		return -1;
	}
